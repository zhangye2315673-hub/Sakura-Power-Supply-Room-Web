import '../skill/SkillCardPresentation.css';
import {
  getSkillCardPreviewDefinitions,
  getSkillCardStyle,
  renderSkillCardSymbol,
  type SkillCardPreviewDefinition,
} from '../skill/SkillCardPresentation';
import './gachaCardPreview.css';

type SkillPolarity = SkillCardPreviewDefinition['polarity'];

const POLARITY_META: Record<SkillPolarity, { title: string; badge: string }> = {
  positive: { title: '正向技能', badge: '正向技能' },
  negative: { title: '负向技能', badge: '负向技能' },
  mixed: { title: '混合技能', badge: '混合技能' },
};

const skillCards = getSkillCardPreviewDefinitions();

function renderCard(card: SkillCardPreviewDefinition): string {
...
const gallery = document.querySelector<HTMLElement>('#skill-card-gallery');
if (!gallery) throw new Error('Missing skill card gallery');

(['positive', 'negative', 'mixed'] as const).forEach((polarity) => {
...
document.querySelector<HTMLElement>('#skill-card-total')!.textContent = String(skillCards.length);

document.querySelectorAll<HTMLButtonElement>('.gacha-preview-card').forEach((card) => {
...
(window as Window & { __GACHA_CARD_PREVIEW_READY__?: boolean }).__GACHA_CARD_PREVIEW_READY__ = true;
