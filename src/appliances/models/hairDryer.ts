...
const V2_REFERENCE_PATH = 'references/intake-v2/hair-dryer/hair-dryer-v2-turnsheet.png';
...
function facetedPanelGeometry(
  ...
  const cut = Math.min(chamfer, halfWidth * 0.42, halfHeight * 0.22);
...
function applyHairDryerOutlineHierarchy(root: THREE.Object3D): void {
...
function nozzleLoftGeometry(): THREE.BufferGeometry {
  const rings: NozzleRing[] = [
    ...
    { x: -1.31, radiusY: 0.73, radiusZ: 0.65 },
    { x: -1.52, radiusY: 0.50, radiusZ: 0.44 },
    { x: -1.78, radiusY: 0.42, radiusZ: 0.33 },
    { x: -2.08, radiusY: 0.55, radiusZ: 0.25 },
...
export function createHairDryerModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const shell = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.78, 0.82, 2.52, 12, 1, false),
    cream,
    bodyPivot,
  );
  ...
  const shellHighlight = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.785, 0.825, 1.84, 12, 1, true, Math.PI * 0.08, Math.PI * 0.5),
    creamHighlight,
    bodyPivot,
    false,
  );
  ...
  const bodySeam = kit.mesh('hair-dryer-body-mould-seam', new THREE.TorusGeometry(0.78, 0.018, 5, 12), pinkShadow, bodyPivot, false);
  bodySeam.rotation.y = Math.PI * 0.5;
  bodySeam.position.x = 0.18;
  bodySeam.userData.explodeWithParent = true;
  const frontLip = kit.mesh('hair-dryer-cream-front-transition-lip', new THREE.TorusGeometry(0.775, 0.07, 6, 12), creamHighlight, bodyPivot, false);
  ...
  const nozzleCouplingSleeve = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.8, 0.78, 0.28, 12, 1, true),
    pinkHighlight,
    nozzlePivot,
    false,
  );
  ...
  const nozzleBand = kit.mesh('hair-dryer-nozzle-socket-band', new THREE.TorusGeometry(0.72, 0.045, 5, 12), pinkHighlight, nozzlePivot, false);
  ...
  const outletRim = kit.mesh('hair-dryer-flat-outlet-rim', new THREE.TorusGeometry(0.57, 0.045, 5, 12), pinkHighlight, nozzlePivot, false);
  ...
  const rearCreamRing = kit.mesh('hair-dryer-rear-cream-bezel', new THREE.TorusGeometry(0.79, 0.115, 6, 12), creamHighlight, rearPivot);
  rearCreamRing.rotation.y = Math.PI * 0.5;
  rearCreamRing.userData.part = 'rear-cream-bezel';
  const rearMintRing = kit.mesh('hair-dryer-rear-mint-intake-ring', new THREE.TorusGeometry(0.64, 0.075, 6, 12), mint, rearPivot);
  rearMintRing.rotation.y = Math.PI * 0.5;
  rearMintRing.position.x = 0.125;
  rearMintRing.userData.part = 'rear-mint-ring';
  const rearCavity = kit.mesh('hair-dryer-rear-mint-perforated-plate', new THREE.CylinderGeometry(0.61, 0.61, 0.045, 12), mintScreen, rearPivot, false);
  ...
  const fanHub = kit.mesh('hair-dryer-fan-hub', new THREE.CylinderGeometry(0.15, 0.15, 0.08, 10), grille, fanPivot, false);
  ...
  const handle = kit.mesh('hair-dryer-pink-rounded-handle-shell', facetedPanelGeometry(0.74, 2.22, 0.66, 0.16), pinkMaterial, handlePivot);
  handle.position.y = -0.84;
  handle.userData.part = 'handle-shell';
  const handleShoulder = kit.mesh(
    'hair-dryer-faceted-handle-shoulder',
    facetedPanelGeometry(0.86, 0.46, 0.68, 0.12),
    pinkHighlight,
    handlePivot,
    false,
  );
  handleShoulder.position.y = 0.16;
  ...
  const coolButtonRing = kit.mesh('hair-dryer-cool-shot-button-ring', new THREE.TorusGeometry(0.15, 0.028, 5, 12), pinkShadow, coolButtonPivot, false);
  coolButtonRing.userData.part = 'cool-shot-control';
  const coolButton = kit.mesh('hair-dryer-round-cool-shot-button', new THREE.CylinderGeometry(0.125, 0.125, 0.055, 10), creamHighlight, coolButtonPivot, false);
  ...
  const collar = kit.mesh('hair-dryer-cream-cable-collar', new THREE.CylinderGeometry(0.34, 0.31, 0.22, 10), cream, cablePivot);
  collar.userData.part = 'cable-collar';
  for (let index = 0; index < 6; index += 1) {
    const topRadius = 0.23 - index * 0.021;
    const rib = kit.mesh(
      `hair-dryer-strain-relief-rib-${index + 1}`,
      new THREE.CylinderGeometry(topRadius - 0.012, topRadius, 0.105, 8),
      cordMaterial,
      cablePivot,
      false,
    );
  ...
  const cord = kit.mesh('hair-dryer-visible-power-cord', new THREE.CylinderGeometry(0.075, 0.075, 0.62, 8), cordMaterial, cablePivot);
  cord.position.y = -0.8;
  ...
  build.root.userData.previewFramingScale = 0.9;
