...
function applyHumidifierOutlineHierarchy(root: THREE.Object3D): void {
  ...
  root.traverse((object) => {
    ...
    const materials = sourceMesh
      ...
      : [];
    const excluded = materials.some((material) => material.transparent)
      || /mist|cloud|lightning|rain|water-glint|inner-water/.test(source);
    if (excluded) {
...
function latheShell(points: readonly (readonly [number, number])[], segments = 12): THREE.LatheGeometry {
...
export function createHumidifierModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const shellMaterial = kit.material(0xf3eddf, { tint: 0x756a7e });
  const shellHighlightMaterial = kit.material(0xfff6e7, { tint: 0x7a6e82 });
  ...
  // V2 macro silhouette: a low twelve-sided base, pinched waist and oversized
  // reservoir give the appliance a graphic game silhouette without changing
  // the archived package or its frozen animation anchors.
  ...
  const tankSeam = kit.mesh(
    ...
    new THREE.TorusGeometry(1.09, 0.028, 6, 12),
    seamMaterial,
    kit.root,
    false,
  );
  ...
  kit.mesh(
    ...
    new THREE.CylinderGeometry(0.34, 0.4, 0.07, 12),
    shellHighlightMaterial,
    outletPivot,
  );
  const outletCap = kit.mesh(
    'humidifier-outlet-rotary-cap',
    new THREE.CylinderGeometry(0.255, 0.295, 0.18, 12),
    shellHighlightMaterial,
    outletPivot,
  );
  ...
  const controlPanel = kit.mesh(
    ...
    new RoundedBoxGeometry(0.68, 0.68, 0.035, 2, 0.11),
    shellHighlightMaterial,
    kit.root,
    false,
  );
  ...
  const dialOuter = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.275, 0.275, 0.07, 12),
    accentDarkMaterial,
    dialPivot,
  );
  ...
  const dial = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.225, 0.235, 0.105, 12),
    accentMaterial,
    dialPivot,
  );
  ...
  const rearPanel = kit.mesh(
    ...
    new RoundedBoxGeometry(0.76, 0.4, 0.06, 2, 0.07),
    shellHighlightMaterial,
  );
  ...
  for (const [index, angle] of [0.42, Math.PI - 0.42, Math.PI + 0.42, -0.42].entries()) {
    const foot = kit.mesh(
      ...
      new THREE.CylinderGeometry(0.11, 0.14, 0.12, 8),
      rubberMaterial,
      kit.root,
      false,
    );
  ...
  build.root.userData.humidifierV2 = {
    ...
    frozenRuntime: ['humidifier-outlet-cap-pivot', 'humidifier-mist-outlet-socket', 'humidifier-control-dial-pivot'],
    envelopePolicy: 'archived v1 package and weather rig are authoritative',
  };
  build.root.userData.sculptRuntime.colliders = [
    { id: 'humidifier-body-envelope', type: 'box', center: [0, 1.405, 0], size: [2.34, 2.81, 2.34] },
  ];
  build.root.userData.sculptRuntime.destructionGroups = [
    { id: 'humidifier-shell-assembly', nodes: ['humidifier-lower-body-shell', 'humidifier-bottom-accent-rail', 'humidifier-front-control-panel'] },
    { id: 'humidifier-reservoir-assembly', nodes: ['humidifier-transparent-water-reservoir', 'humidifier-inner-water-volume', 'humidifier-tank-body-seam'] },
    { id: 'humidifier-outlet-assembly', nodes: ['humidifier-outlet-cap-pivot', 'humidifier-outlet-recess-seat', 'humidifier-outlet-rotary-cap'] },
  ];
