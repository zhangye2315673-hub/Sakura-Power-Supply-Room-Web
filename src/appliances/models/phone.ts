...
function applyPhoneOutlineHierarchy(root: THREE.Object3D): void {
...
function chamferedSlabGeometry(
  width: number,
  height: number,
  depth: number,
  corner: number,
...
export function createPhoneModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const shellShade = kit.material(0xe7dbe2, { tint: 0x675b72 });

  const handsetPivot = kit.pivot('phone-handset-pivot');
  kit.socket('phone-body-socket', handsetPivot, [0, 0.96, 0]);

  // V2 blockout: an eight-corner faceted slab keeps the archived envelope but
  // replaces the soft generic rectangle with a chunky handheld-game silhouette.
  const perimeterRail = kit.mesh(
    'phone-accent-perimeter-rail',
    chamferedSlabGeometry(1.02, 1.92, 0.17, 0.13),
    rail,
    handsetPivot,
  );
  ...
  const rearShell = kit.mesh(
    ...
    chamferedSlabGeometry(0.974, 1.87, 0.14, 0.115),
    cream,
    handsetPivot,
    false,
  );
  ...
  for (const [name, x, y, rotation] of [
    ['top-left', -0.405, 1.805, -0.18],
    ['top-right', 0.405, 1.805, 0.18],
    ['bottom-left', -0.405, 0.155, 0.18],
    ['bottom-right', 0.405, 0.155, -0.18],
  ] as const) {
    const guard = kit.mesh(
      `phone-faceted-corner-guard-${name}`,
      new THREE.CylinderGeometry(0.105, 0.125, 0.19, 6),
      railDark,
      handsetPivot,
      false,
    );
    guard.rotation.set(Math.PI * 0.5, 0, rotation);
    guard.position.set(x, y, 0);
    guard.scale.set(1, 0.66, 1);
    guard.userData.part = 'corner-guards';
  }

  const rearSpine = kit.mesh(
    'phone-rear-faceted-center-spine',
    chamferedSlabGeometry(0.17, 1.36, 0.018, 0.04),
    shellShade,
    handsetPivot,
    false,
  );
  rearSpine.position.set(0.28, 0.87, -0.096);
  ...
  const frontBezel = kit.mesh(
    ...
    chamferedSlabGeometry(0.94, 1.8, 0.035, 0.1),
    creamLight,
    screenPivot,
    false,
  );
  ...
  const screen = kit.mesh(
    ...
    chamferedSlabGeometry(0.875, 1.71, 0.032, 0.082),
    screenGlow,
    screenPivot,
    false,
  );
  ...
  const screenCrown = kit.mesh(
    'phone-low-poly-screen-crown',
    chamferedSlabGeometry(0.54, 0.08, 0.025, 0.028),
    rail,
    screenPivot,
  );
  screenCrown.position.set(0, 0.79, 0.046);
  screenCrown.userData.part = 'screen-trim';
  const screenChin = kit.mesh(
    'phone-low-poly-screen-chin',
    chamferedSlabGeometry(0.42, 0.055, 0.025, 0.02),
    rail,
    screenPivot,
    false,
  );
  screenChin.position.set(0, -0.81, 0.046);
  screenChin.userData.part = 'screen-trim';

  for (const side of [-1, 1] as const) {
    const grip = kit.mesh(
      `phone-faceted-side-grip-${side < 0 ? 'left' : 'right'}`,
      new THREE.CylinderGeometry(0.052, 0.068, 0.54, 6),
      railDark,
      handsetPivot,
      false,
    );
    grip.position.set(side * 0.485, 0.88, -0.004);
    grip.scale.set(0.72, 1, 1);
  ...
  const cameraIsland = kit.mesh(
    ...
    chamferedSlabGeometry(0.42, 0.5, 0.045, 0.075),
    rail,
    cameraPivot,
    false,
  );
  ...
  for (const [index, x, y] of [
    ...
    const ring = kit.mesh(
      ...
      new THREE.TorusGeometry(0.092, 0.019, 5, 12),
      metal,
      lensPivot,
      false,
    );
  ...
  for (let index = 0; index < 10; index += 1) {
    ...
    const radius = index % 2 === 0 ? 0.055 : 0.022;
  ...
  const petalBadge = kit.mesh(
    'phone-rear-sakura-five-petal-badge',
    new THREE.ExtrudeGeometry(petalShape, { depth: 0.012, steps: 1, bevelEnabled: false, curveSegments: 1 }),
    rail,
    handsetPivot,
    false,
  );
  petalBadge.position.set(0, 0.73, -0.114);
  ...
  build.root.userData.sculptRuntime.destructionGroups.push(
    {
      ...
      nodes: ['phone-accent-perimeter-rail', 'phone-cream-rear-shell', 'phone-front-cream-bezel'],
    },
    {
      id: 'phone-camera-assembly',
      nodes: ['phone-rear-camera-island-pivot', 'phone-rear-camera-lens-1-pivot', 'phone-rear-camera-lens-2-pivot'],
    },
  );
