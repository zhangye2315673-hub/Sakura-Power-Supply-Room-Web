...
function applyLampOutlineHierarchy(root: THREE.Object3D): void {
...
export function createLampModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const lowerRing = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.845, 0.865, 0.105, 12, 1),
    accentDarkMaterial,
  );
  ...
  const baseBody = kit.mesh(
    'lamp-base-body',
    new THREE.CylinderGeometry(0.795, 0.845, 0.145, 12, 2),
    accentMaterial,
  );
  ...
  const baseTop = kit.mesh(
    'lamp-base-top-plate',
    new THREE.CylinderGeometry(0.705, 0.795, 0.075, 12, 1),
    accentLightMaterial,
  );
  ...
  const reflector = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.29, 0.545, 0.27, 12, 2, true),
    reflectorMaterial,
    headPivot,
  );
  ...
  const diffuser = kit.mesh(
    'lamp-warm-diffuser',
    new THREE.CylinderGeometry(0.565, 0.565, 0.035, 12),
    diffuserMaterial,
    headPivot,
  );
  ...
  result.root.userData.sculptRuntime.colliders = [
    { id: 'lamp-body', type: 'box', node: 'root' },
    { id: 'lamp-head', type: 'box', node: 'lamp-head-hinge-pivot' },
    { id: 'lamp-base', type: 'cylinder', node: 'lamp-base-body' },
  ];
  result.root.userData.sculptRuntime.destructionGroups = [
    { id: 'lamp-base-system', nodes: ['lamp-base-lower-ring', 'lamp-base-body', 'lamp-base-top-plate', 'lamp-base-foot-left', 'lamp-base-foot-right'] },
    { id: 'lamp-stem-system', nodes: ['lamp-stem-foot-collar', 'lamp-stem', 'lamp-stem-top-socket'] },
    { id: 'lamp-hinge-system', nodes: ['lamp-stem-hinge-bridge', 'lamp-hinge-yoke', 'lamp-hinge-axle', 'lamp-hinge-cap-left', 'lamp-hinge-cap-right'] },
    { id: 'lamp-head-system', nodes: ['lamp-shade-shell', 'lamp-shade-top-cap', 'lamp-shade-front-rim', 'lamp-inner-reflector', 'lamp-bulb-volume', 'lamp-warm-diffuser', 'lamp-shade-top-button-pivot'] },
  ];
