...
const V2_REFERENCE_PATH = 'references/intake-v2/kettle/views/front.png';
...
function applyKettleOutlineHierarchy(root: THREE.Object3D): void {
...
function lathe(points: readonly (readonly [number, number])[], segments = 12): THREE.LatheGeometry {
...
function tubeThrough(points: readonly THREE.Vector3[], radius: number): THREE.TubeGeometry {
  return new THREE.TubeGeometry(new THREE.CatmullRomCurve3([...points], false, 'centripetal'), 24, radius, 8, false);
}
...
export function createKettleModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const gauge = kit.material(0xc5e3df, { tint: 0x6d8d94, transparent: true, opacity: 0.62 });
  ...
  const bodySeam = kit.mesh(
    ...
    new THREE.TorusGeometry(0.845, 0.018, 6, 12),
    seam,
    bodyPivot,
    false,
  );
  ...
  const base = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.89, 0.84, 0.2, 12),
    shellLight,
    powerBase,
  );
  ...
  const baseAccent = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.9, 0.89, 0.07, 12),
    accentDeep,
    powerBase,
  );
  ...
  const lid = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.49, 0.57, 0.16, 12),
    shellLight,
    lidPivot,
  );
  ...
  const lidSeat = kit.mesh(
    ...
    new THREE.TorusGeometry(0.53, 0.022, 6, 12),
    seam,
    lidPivot,
    false,
  );
  ...
  const spout = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.16, 0.32, 1, 10, 1, false),
    metal,
    spoutPivot,
  );
  ...
  build.root.userData.previewFramingScale = 0.94;
  build.root.userData.legacyReferencePath = REFERENCE_PATH;
  build.root.userData.referenceDimensions = {
    lidHinge: [0, 1.93, -0.48],
    spoutBodySocket: [-0.62, 1.52, 0.02],
    switchPivot: [0.12, 0.5, 0.86],
    rearPowerSocket: [0, 0.13, -0.905],
  };
  build.root.userData.sculptRuntime.colliders = [
    { id: 'kettle-body', type: 'cylinder', node: 'kettle-body-pivot' },
    { id: 'kettle-lid', type: 'cylinder', node: 'kettle-lid-hinge-pivot', trigger: true },
    { id: 'kettle-base', type: 'cylinder', node: 'kettle-power-base-pivot' },
  ];
  build.root.userData.sculptRuntime.destructionGroups = [
    ['kettle-body-pivot', 'kettle-front-water-level-window', 'kettle-power-switch-pivot'],
