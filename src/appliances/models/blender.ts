...
const REFERENCE_PATH = 'references/intake-v2/blender/views/front.png';
...
function applyBlenderOutlineHierarchy(root: THREE.Object3D): void {
...
function handleGeometry(): THREE.TubeGeometry {
  const curve = new THREE.CatmullRomCurve3([
    ...
    new THREE.Vector3(1.91024, 1.54, -0.02),
    new THREE.Vector3(1.91024, 0.72, -0.02),
    new THREE.Vector3(1.72, 0.38, -0.02),
    new THREE.Vector3(1.12, 0.18, -0.02),
  ]);
  return new THREE.TubeGeometry(curve, 12, 0.18, 8, false);
}
...
function jarRibGeometry(): THREE.TubeGeometry {
  ...
  return new THREE.TubeGeometry(curve, 8, 0.034, 5, false);
}
...
function appleGeometry(): THREE.BufferGeometry {
  const geometry = new THREE.SphereGeometry(0.255, 10, 7);
  const positions = geometry.attributes.position;
  for (let index = 0; index < positions.count; index += 1) {
    const x = positions.getX(index);
    const y = positions.getY(index);
...
function bananaGeometry(): THREE.TubeGeometry {
  ...
  return new THREE.TubeGeometry(curve, 12, 0.105, 6, false);
}
...
export function createBlenderModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const jarSeat = kit.mesh(
    ...
    new THREE.CylinderGeometry(1.01, 1.08, 0.14, 12),
    pinkEdge,
    jarSeatPivot,
  );
  ...
  const upperCollar = kit.mesh(
    ...
    new THREE.CylinderGeometry(1.265, 1.2, 0.34, 12, 1, true),
    collarMaterial,
    jarPivot,
    false,
  );
  ...
  const jarRim = kit.mesh(
    ...
    new THREE.TorusGeometry(1.265, 0.035, 6, 12),
    jarHighlight,
    jarPivot,
    false,
  );
  ...
  const coupling = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.31, 0.38, 0.12, 12),
    cavity,
    jarPivot,
  );
  ...
  const bladeHub = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.21, 0.26, 0.2, 10),
    steel,
    bladePivot,
  );
  ...
  const liquidGeometry = new THREE.CylinderGeometry(1.15, 0.79, 2.18, 12, 2, false);
  ...
  const spiral = kit.mesh(
    ...
    new THREE.TubeGeometry(spiralCurve, 32, 0.026, 5, false),
    smoothieHighlight,
    vortexPivot,
    false,
  );
  ...
  fruitSeeds.forEach((entry) => {
    ...
    if (entry.id === 'strawberry') {
      ...
      const leaf = kit.mesh('blender-fruit-apple-leaf', new THREE.SphereGeometry(0.1, 7, 4), fruitGreen, fruitPivot, false);
      leaf.position.set(0.11, 0.32, 0);
      leaf.scale.set(1.45, 0.2, 0.66);
      leaf.rotation.z = -0.38;
      setPart(leaf, 'powered-fruit-assembly', true);
    } else if (entry.id === 'orange') {
      const body = kit.mesh('blender-fruit-orange-dimpled-body', new THREE.IcosahedronGeometry(0.265, 1), fruitOrange, fruitPivot);
      setPart(body, 'powered-fruit-assembly');
      ...
      for (const [x, y, label] of [[-0.36, 0.08, 'left'], [0.37, 0.12, 'right']] as const) {
        const cap = kit.mesh(`blender-fruit-banana-${label}-cap`, new THREE.SphereGeometry(0.11, 7, 4), fruitYellow, fruitPivot, false);
  ...
  [[-0.11, 0.06, 0.03], [0.1, 0.08, -0.02], [0, 0.24, 0.04]].forEach((position, index) => {
    const berry = kit.mesh(`blender-fruit-blueberry-${index + 1}-body`, new THREE.DodecahedronGeometry(0.135, 0), blueberry, berryPivot);
    berry.position.set(position[0], position[1], position[2]);
    setPart(berry, 'powered-fruit-assembly');
    const crown = kit.mesh(`blender-fruit-blueberry-${index + 1}-crown`, new THREE.TorusGeometry(0.042, 0.012, 4, 6), blueberryBloom, berryPivot, false);
  ...
  const handleRootGeometry = new THREE.SphereGeometry(1, 8, 5);
  ...
  const lidFlange = kit.mesh(
    ...
    new THREE.TorusGeometry(1.34, 0.045, 6, 12),
    pinkEdge,
    lidPivot,
    false,
  );
  ...
  const lidKnob = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.46, 0.48, 0.14, 10),
    mint,
    lidPivot,
  );
  ...
  const dial = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.45, 0.47, 0.16, 12),
    mint,
    dialPivot,
  );
  ...
  const dialShadowRing = kit.mesh(
    ...
    new THREE.TorusGeometry(0.475, 0.035, 6, 12),
    pinkShadow,
    dialPivot,
    false,
  );
  ...
  const dialFaceRing = kit.mesh(
    ...
    new THREE.TorusGeometry(0.405, 0.018, 5, 12),
    creamLight,
    dialPivot,
    false,
  );
  ...
  const rearPanel = kit.mesh(
    ...
    // Reach the tapered shell's rear plane while keeping the whole assembly
    // inside the archived v1 Box3. The former 0.035-deep panel sat fully
    // inside the base and made the rear-service contract invisible.
    rounded(0.92, 0.5, 0.28, 0.08, 3),
    creamLight,
    rearServicePivot,
    false,
  );
  ...
  for (let index = 0; index < 5; index += 1) {
    ...
    // The tapered shoulder recedes at different depths. Place every slot on
    // the shared rear-most service plane so all five remain separately
    // readable in the strict back view instead of being swallowed in pairs.
    vent.position.set(0, 1.13 + index * 0.095, -1.47);
    setPart(vent, 'rear-vent-array', true);
  }

  const inletFrame = kit.mesh(
    'blender-rear-power-inlet-frame',
    rounded(0.48, 0.28, 0.32, 0.065, 3),
    pinkShadow,
    rearServicePivot,
  );
  inletFrame.position.set(0, 0.42, -1.33);
  ...
  inletCore.position.set(0, 0.42, -1.488);
  ...
  for (const [x, label] of [[-0.08, 'left'], [0.08, 'right']] as const) {
    const pin = kit.mesh(
      ...
      new THREE.SphereGeometry(0.025, 8, 5),
      rubber,
      rearServicePivot,
      false,
    );
    pin.position.set(x, 0.42, -1.482);
    setPart(pin, 'rear-power-inlet', true);
  }
  kit.socket('blender-power-cable-socket', rearServicePivot, [0, 0.42, -1.16]);
  // Freeze scene-edge cable targets to the archived v1 Box3 fallback. These
  // zero-geometry sockets are resolved before ApplianceScene recenters the
  // model, so they intentionally use the original root-local coordinates.
  kit.socket('blender-left-connection-socket', kit.root, [-1.64728263, 2.42, 1.6975]);
  kit.socket('blender-right-connection-socket', kit.root, [2.15408197, 2.42, 1.6975]);
  kit.socket('blender-top-connection-socket', kit.root, [0.25339967, 4.835, 1.6975]);
  kit.socket('blender-bottom-connection-socket', kit.root, [0.25339967, 0.005, 1.6975]);
  ...
  // Review-only light rig used by the v2 appliance family. The gameplay
  // scene keeps its existing shared city lighting; only model-review renders
  // opt into the softer key/fill/rim profile for consistent turn-sheets.
