...
const V2_REFERENCE_PATH = 'references/intake-v2/induction-cooktop/views/front.png';
...
function applyInductionCooktopOutlineHierarchy(root: THREE.Object3D): void {
...
export function createInductionCooktopModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const accentDeep = kit.material(accent.clone().offsetHSL(-0.015, 0.04, -0.13), { tint: 0x6e5268 });
  ...
  for (const [x, z, rotationY, label] of [
    [0, 1.64, 0, 'front'],
    [0, -1.64, 0, 'rear'],
    [-1.71, 0, Math.PI * 0.5, 'left'],
    [1.71, 0, Math.PI * 0.5, 'right'],
  ] as const) {
    const shoulder = kit.mesh(
      `induction-cooktop-faceted-shell-shoulder-${label}`,
      rounded(label === 'front' || label === 'rear' ? 2.82 : 2.67, 0.12, 0.15, 0.045),
      creamHighlight,
      enclosurePivot,
    );
    shoulder.position.set(x, 0.54, z);
  ...
  const cookingZoneSeat = kit.mesh(
    'induction-cooktop-faceted-cooking-zone-seat',
    new THREE.CylinderGeometry(1.22, 1.25, 0.045, 12),
    accentDeep,
    topPivot,
  );
  cookingZoneSeat.position.set(0, 0.755, -0.28);
  markPart(cookingZoneSeat, 'heating-zone');

  const cookingZoneInset = kit.mesh(
    'induction-cooktop-faceted-cooking-zone-inset',
    new THREE.CylinderGeometry(1.18, 1.2, 0.052, 12),
    glass,
    topPivot,
    false,
  );
  cookingZoneInset.position.set(0, 0.785, -0.28);
  ...
  const buttonGeometry = new THREE.CylinderGeometry(0.16, 0.145, 0.065, 12);
  const buttonRingGeometry = new THREE.TorusGeometry(0.152, 0.022, 5, 12);
  ...
  const knobShadow = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.32, 0.3, 0.105, 12),
    ink,
    knobPivot,
  );
  ...
  const knob = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.27, 0.29, 0.135, 12),
    creamHighlight,
    knobPivot,
  );
  ...
  for (let index = 0; index < 8; index += 1) {
    const grip = kit.mesh(
      `induction-cooktop-knob-grip-ridge-${index + 1}`,
      rounded(0.025, 0.055, 0.105, 0.008),
      accentDeep,
      knobPivot,
      false,
    );
    const angle = index * Math.PI * 2 / 8;
    grip.position.set(Math.cos(angle) * 0.27, 0.075, Math.sin(angle) * 0.27);
  ...
  const panBottom = kit.mesh(
    ...
    new THREE.CylinderGeometry(1.11, 1.03, 0.16, 12),
    ink,
    cookwarePivot,
  );
  ...
  const panWall = kit.mesh(
    ...
    new THREE.CylinderGeometry(1.18, 1.1, 0.7, 12, 1, true),
    creamHighlight,
    cookwarePivot,
  );
  ...
  const panInnerWall = kit.mesh(
    ...
    new THREE.CylinderGeometry(1.07, 1.02, 0.58, 12, 1, true),
    potInterior,
    cookwarePivot,
    false,
  );
  ...
  const panRim = kit.mesh(
    ...
    new THREE.TorusGeometry(1.125, 0.065, 6, 12),
    ink,
    cookwarePivot,
  );
  ...
  const soup = kit.mesh(
    ...
    new THREE.CylinderGeometry(1.03, 1.03, 0.035, 12),
    soupMaterial,
    cookwarePivot,
    false,
  );
