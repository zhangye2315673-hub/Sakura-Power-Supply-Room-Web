...
const V2_REFERENCE_PATH = 'references/intake-v2/robot-vacuum/views/front.png';
...
function applyRobotVacuumOutlineHierarchy(root: THREE.Object3D): void {
...
export function createRobotVacuumModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const lowerChassis = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.91, 0.96, 0.31, 12),
    cream,
    chassisPivot,
  );
  ...
  const lowerSkirt = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.89, 0.92, 0.075, 12),
    rubber,
    chassisPivot,
  );
  ...
  const topRim = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.89, 0.93, 0.12, 12),
    creamLight,
    topAssembly,
  );
  ...
  const topCover = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.81, 0.865, 0.085, 12),
    accentLight,
    topAssembly,
  );
  ...
  const lidarSeat = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.21, 0.23, 0.045, 12),
    cream,
    lidarPivot,
  );
  ...
  const lidarTower = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.17, 0.185, 0.15, 12),
    creamLight,
    lidarPivot,
  );
  lidarTower.position.y = 0.068;
  lidarTower.userData.part = 'lidar-tower';
  const lidarCap = kit.mesh(
    'robot-vacuum-lidar-accent-cap',
    new THREE.CylinderGeometry(0.178, 0.17, 0.045, 12),
    accent,
    lidarPivot,
  );
  lidarCap.position.y = 0.158;
  const lidarWindow = kit.mesh(
    'robot-vacuum-lidar-window',
    new THREE.BoxGeometry(0.15, 0.06, 0.018),
    sensor,
    lidarPivot,
    false,
  );
  lidarWindow.position.set(0, 0.075, 0.184);
  addHorizontalRing(
    kit,
  ...
  const undersidePlate = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.76, 0.8, 0.055, 12),
    creamLight,
    undersidePivot,
  );
  ...
  build.root.userData.sculptRuntime.colliders = [
    { id: 'robot-vacuum-chassis-collider', type: 'cylinder', node: 'robot-vacuum-chassis-pivot' },
    { id: 'robot-vacuum-bumper-collider', type: 'compound', node: 'robot-vacuum-front-bumper-pivot' },
  ];
  build.root.userData.sculptRuntime.destructionGroups = [
    { id: 'robot-vacuum-chassis-group', nodes: ['robot-vacuum-chassis-pivot'] },
    { id: 'robot-vacuum-top-group', nodes: ['robot-vacuum-top-assembly-pivot'] },
    { id: 'robot-vacuum-bumper-group', nodes: ['robot-vacuum-front-bumper-pivot'] },
    { id: 'robot-vacuum-cleaning-group', nodes: ['robot-vacuum-underside-assembly-pivot'] },
  ];
  return build;
}
