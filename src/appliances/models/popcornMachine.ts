...
const V2_REFERENCE_PATH = 'references/intake-v2/popcorn-machine/views/front.png';
void REFERENCE_PATH;
...
function applyPopcornMachineOutlineHierarchy(root: THREE.Object3D): void {
...
function facetedCabinetGeometry(): THREE.BufferGeometry {
  const shape = new THREE.Shape();
  shape.moveTo(-0.91, -0.63);
  shape.lineTo(0.91, -0.63);
  shape.lineTo(1.01, -0.5);
  shape.lineTo(1.01, 0.42);
  shape.lineTo(0.89, 0.63);
  shape.lineTo(-0.89, 0.63);
  shape.lineTo(-1.01, 0.42);
  shape.lineTo(-1.01, -0.5);
  shape.closePath();
...
export function createPopcornMachineModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  for (const x of [-0.86, 0.86]) {
    const cornerBlade = kit.mesh(
      `popcorn-machine-cabinet-corner-blade-${x < 0 ? 'left' : 'right'}`,
      new THREE.BoxGeometry(0.16, 0.82, 0.055, 1, 3, 1),
      accentSoft,
      cabinetPivot,
    );
    cornerBlade.position.set(x, 0.78, 0.677);
    cornerBlade.rotation.z = x < 0 ? -0.06 : 0.06;
    cornerBlade.userData.part = 'cabinet-corner-blades';
  }

  const marqueePanel = kit.mesh(
    'popcorn-machine-front-marquee-panel',
    new RoundedBoxGeometry(0.58, 0.1, 0.045, 2, 0.02),
    accentDeep,
    cabinetPivot,
  );
  marqueePanel.position.set(-0.1, 1.22, 0.715);
  ...
  for (const x of [-0.7, 0, 0.7]) {
    const roofRib = kit.mesh(
      `popcorn-machine-dome-rib-${x < 0 ? 'left' : x > 0 ? 'right' : 'center'}`,
      wedgeGeometry(),
      x === 0 ? accentDeep : accent,
      domePivot,
    );
    roofRib.position.set(x, 0.25, 0.28);
    roofRib.scale.set(x === 0 ? 0.42 : 0.34, 0.42, 0.72);
    roofRib.rotation.z = x * -0.12;
  ...
  for (const [x, label] of [[-0.78, 'left'], [0.78, 'right']] as const) {
    const windowBrace = kit.mesh(
      `popcorn-machine-front-window-brace-${label}`,
      new THREE.BoxGeometry(0.085, 1.05, 0.055, 1, 4, 1),
      accentSoft,
      chamberPivot,
    );
    windowBrace.position.set(x, 0.75, 0.626);
    windowBrace.rotation.z = x < 0 ? -0.025 : 0.025;
  ...
  const controlButton = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.145, 0.155, 0.105, 12),
    accentSoft,
    controlPivot,
  );
  ...
  result.root.userData.sculptRuntime.destructionGroups = [
    ...
    ['popcorn-machine-cabinet-corner-blade-left', 'popcorn-machine-cabinet-corner-blade-right', 'popcorn-machine-front-marquee-panel'],
    ['popcorn-machine-clear-front-panel', 'popcorn-machine-clear-back-panel'],
    ['popcorn-machine-chamber-post-front-left', 'popcorn-machine-chamber-post-front-right', 'popcorn-machine-front-window-brace-left', 'popcorn-machine-front-window-brace-right'],
    ['popcorn-machine-low-pink-dome', 'popcorn-machine-top-crown', 'popcorn-machine-dome-rib-left', 'popcorn-machine-dome-rib-center', 'popcorn-machine-dome-rib-right'],
    ['popcorn-machine-deep-arched-delivery-chute', 'popcorn-machine-projecting-delivery-tray'],
    ['popcorn-machine-control-dark-seat', 'popcorn-machine-round-control-button', 'popcorn-machine-control-index-mark'],
  ];
  return result;
}
