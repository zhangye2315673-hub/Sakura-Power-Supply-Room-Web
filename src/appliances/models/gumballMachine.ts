...
function lidDomeGeometry(): THREE.SphereGeometry {
  const geometry = new THREE.SphereGeometry(1.01, 12, 5, 0, Math.PI * 2, 0, Math.PI * 0.5);
...
export function createGumballMachineModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  for (const [index, x, z, rotationY] of [
    ...
    const shoulder = kit.mesh(
      `gumball-machine-pink-shoulder-bracket-${index}`,
      new THREE.BoxGeometry(0.2, 0.43, 0.16),
      accentSoft,
      basePivot,
    );
    shoulder.position.set(x, 0.47, z);
  ...
  const globeShell = kit.mesh(
    ...
    new THREE.SphereGeometry(1.38, 12, 8),
    globe,
    globePivot,
    false,
  );
  ...
  const frontContour = kit.mesh(
    ...
    new THREE.TorusGeometry(1.49, 0.009, 4, 24),
    globeOutline,
    globePivot,
    false,
  );
  ...
  const sideContour = kit.mesh(
    ...
    new THREE.TorusGeometry(1.38, 0.009, 4, 24),
    globeOutline,
    globePivot,
    false,
  );
  ...
  const lidRim = kit.mesh(
    ...
    new THREE.TorusGeometry(1.02, 0.075, 5, 12),
    accentDeep,
    topLidPivot,
  );
  ...
  const lowerRingHighlight = kit.mesh(
    ...
    new THREE.TorusGeometry(0.99, 0.034, 4, 12),
    accentSoft,
    globePivot,
    false,
  );
  ...
  const dividerHub = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.21, 0.23, 0.09, 10),
    accentDeep,
    globePivot,
  );
  ...
  capsuleData.forEach(([x, y, z, rx, rz, palette], index) => {
    ...
    const upper = kit.mesh(
      ...
      new THREE.SphereGeometry(0.255, 12, 6, 0, Math.PI * 2, 0, Math.PI * 0.5),
      capsuleClear,
      capsulePivot,
      false,
    );
    ...
    const lower = kit.mesh(
      ...
      new THREE.SphereGeometry(0.255, 12, 6, 0, Math.PI * 2, Math.PI * 0.5, Math.PI * 0.5),
  ...
  const baseRing = kit.mesh(
    ...
    new THREE.CylinderGeometry(1.065, 1.075, 0.2, 12),
    accent,
    basePivot,
  );
  ...
  const crankSeat = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.35, 0.38, 0.12, 12),
    accentDeep,
    frontCrankPivot,
  );
  ...
  const crankFace = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.29, 0.31, 0.15, 12),
    accent,
    frontCrankPivot,
  );
  ...
  const frontButton = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.13, 0.14, 0.075, 10),
    accent,
    basePivot,
  );
  ...
  const sideHub = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.17, 0.19, 0.12, 10),
    accentDeep,
    sideCrankPivot,
  );
  ...
  const rearNotch = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.16, 0.16, 0.1, 10, 1, false, 0, Math.PI),
    cavity,
    basePivot,
    false,
  );
  ...
  const runtime = build.root.userData.sculptRuntime as {
    colliders: Array<{ id: string; type: string; node: string; trigger?: boolean }>;
    destructionGroups: Array<{ id: string; nodes: string[] }>;
  };
  runtime.colliders = [
    { id: 'gumball-machine-pedestal', type: 'cylinder', node: 'gumball-machine-base-pivot' },
    { id: 'gumball-machine-globe', type: 'sphere', node: 'gumball-machine-globe-assembly-pivot' },
    { id: 'gumball-machine-dispense-trigger', type: 'box', node: 'gumball-machine-dispense-chute-pivot', trigger: true },
  ];
  runtime.destructionGroups = [
    { id: 'globe-assembly', nodes: ['gumball-machine-transparent-globe-shell', 'gumball-machine-domed-top-lid', 'gumball-machine-globe-seat-ring', 'gumball-machine-internal-center-column'] },
    { id: 'pedestal-assembly', nodes: ['gumball-machine-rounded-frustum-base-shell', 'gumball-machine-pink-lower-base-ring', 'gumball-machine-pink-shoulder-bracket-1', 'gumball-machine-pink-shoulder-bracket-2', 'gumball-machine-pink-shoulder-bracket-3', 'gumball-machine-pink-shoulder-bracket-4'] },
