...
function applyFanOutlineHierarchy(root: THREE.Object3D): void {
...
export function createFanModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  // Freeze the four scene-edge anchors to the archived fan envelope. The
  // visual head can rotate, but these sockets remain root children so cable
  // projection and collision logic do not drift with animation.
  kit.socket('fan-left-connection-socket', kit.root, [-0.69, 0.00957517, 0.41]);
  kit.socket('fan-right-connection-socket', kit.root, [0.69, 0.00957517, 0.41]);
  kit.socket('fan-top-connection-socket', kit.root, [0, 0.94590842, 0.41]);
  kit.socket('fan-bottom-connection-socket', kit.root, [0, -0.92675808, 0.41]);
  ...
  build.root.userData.referenceDimensions = {
    envelope: { min: [-0.655, -0.89175808, -0.61652837], max: [0.655, 0.91090842, 0.37499997] },
    guardOuterRadius: 0.62,
    bladeCount: 5,
    guardRingCount: 3,
  };
  build.root.userData.previewLightingProfile = 'sakura-appliance-v2';
  build.root.userData.previewFramingScale = 0.92;
  ...
  build.root.userData.sculptRuntime.colliders = [
    { id: 'fan-base', type: 'box', node: 'fan-base-shell' },
    { id: 'fan-head', type: 'sphere', node: 'fan-head-hinge', radius: 0.66 },
  ];
  build.root.userData.sculptRuntime.destructionGroups = [
    { id: 'fan-base-assembly', nodes: ['fan-base-shell', 'fan-base-accent-rail', 'fan-speed-dial-pivot'] },
    { id: 'fan-support-assembly', nodes: ['fan-support-column', 'fan-support-collar', 'fan-oscillation-pivot'] },
    { id: 'fan-head-assembly', nodes: ['fan-head-hinge', 'fan-motor-housing', 'fan-front-guard', 'fan-rear-guard'] },
    { id: 'fan-rotor-assembly', nodes: ['fan-rotor-pivot'] },
  ];
  return build;
}
