...
const V2_REFERENCE_PATH = 'references/intake-v2/refrigerator/views/front.png';
...
function applyRefrigeratorOutlineHierarchy(root: THREE.Object3D): void {
...
export function createRefrigeratorModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const mintAccentMaterial = kit.material(0xa9d3cb, { tint: 0x667c82 });
  ...
  const basePlinth = kit.mesh(
    ...
    new RoundedBoxGeometry(2.72, 0.22, 1.82, 1, 0.07),
    accentMaterial,
  );
  basePlinth.position.set(0, 0.26, -0.01);

  const steppedCrown = roundedBar(
    kit,
    'refrigerator-stepped-crown',
    2.68,
    0.16,
    1.82,
    0.06,
  ...
  steppedCrown.position.set(0, 5.035, -0.01);
  const crownAccentRail = roundedBar(
    kit,
    'refrigerator-crown-accent-rail',
    2.55,
    0.055,
    1.8,
    0.02,
  ...
  crownAccentRail.position.set(0, 4.935, -0.005);

  for (const [side, x] of [['left', -1.405], ['right', 1.405]] as const) {
    const sideFacet = kit.mesh(
      `refrigerator-side-facet-${side}`,
      new THREE.BoxGeometry(0.025, 4.38, 1.52),
      shellShadowMaterial,
      cabinetFrame,
      false,
    );
    sideFacet.position.set(x, 2.68, -0.08);
  ...
  const upperDoorFace = kit.mesh(
    'refrigerator-upper-door-face-border',
    facetedPanelGeometry(2.53, 1.4, 0.055, 0.14),
    shellMaterial,
    upperDoorPivot,
  );
  upperDoorFace.position.set(-1.365, 0, 0.235);
  ...
  const lowerDoorFace = kit.mesh(
    'refrigerator-lower-door-face-border',
    facetedPanelGeometry(2.53, 2.7, 0.055, 0.15),
    shellMaterial,
    lowerDoorPivot,
  );
  lowerDoorFace.position.set(-1.365, 0, 0.235);
  ...
  const storageBody = roundedBar(kit, 'refrigerator-prop-storage-box-body', 0.66, 0.36, 0.5, 0.07, storageMaterial, storage, false);
  storageBody.userData.explodeWithParent = true;
  const storageLid = roundedBar(kit, 'refrigerator-prop-storage-box-lid', 0.72, 0.09, 0.54, 0.045, storageLidMaterial, storage, false);
  ...
  function addHandle(
    ...
    const grip = kit.mesh(
      `${prefix}-grip`,
      new THREE.CylinderGeometry(0.075, 0.075, height, 8),
      accentLightMaterial,
      handlePivot,
    );
    grip.scale.set(1.4, 1, 0.86);
    grip.position.z = 0.075;
    grip.userData.explodeWithParent = true;
    const channel = roundedBar(
      kit,
      `${prefix}-shadow-channel`,
      0.16,
      height + 0.09,
      0.025,
      0.035,
    ...
    channel.position.z = -0.045;
    const mintInsert = roundedBar(
      kit,
      `${prefix}-mint-insert`,
      0.055,
      height * 0.68,
  ...
  for (const [index, y] of [4.84, 3.75, 2.82, 0.74].entries()) {
    const hingeCap = kit.mesh(
      ...
      new RoundedBoxGeometry(0.13, 0.18, 0.16, 1, 0.035),
      accentMaterial,
      kit.root,
    );
  ...
  const servicePanel = kit.mesh(
    'refrigerator-rear-service-panel',
    facetedPanelGeometry(2.3, 0.76, 0.07, 0.1),
    accentLightMaterial,
    compressorPivot,
    false,
  );
  ...
  // Freeze the archived v1 Box3 fallback targets as explicit zero-geometry
  // sockets. They are resolved before ApplianceScene recenters the model, so
  // thicker outlines and the new faceted silhouette cannot move cable ends.
  kit.socket('refrigerator-left-connection-socket', kit.root, [-1.455, 2.5725, 1.485]);
  kit.socket('refrigerator-right-connection-socket', kit.root, [1.5, 2.5725, 1.485]);
  kit.socket('refrigerator-top-connection-socket', kit.root, [0.0225, 5.18, 1.485]);
  kit.socket('refrigerator-bottom-connection-socket', kit.root, [0.0225, -0.035, 1.485]);
