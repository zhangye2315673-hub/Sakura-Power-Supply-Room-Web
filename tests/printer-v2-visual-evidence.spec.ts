...
const OUTPUT_DIR = path.resolve('artifacts/appliance-v2/printer/evidence/models/printer');
...
test('captures printer v2 static views and frozen paper animation phases', async ({ page }) => {
  ...
  for (const view of VIEWS) {
    await page.goto(`/model-review.html?model=printer&view=${view}&spin=0&power=0`);
    ...
    expect(diagnostics?.drawCalls ?? 0).toBeGreaterThan(20);
    captures.push({ state: 'idle', view, screenshotPath, diagnostics });
  }

  await page.goto('/model-review.html?model=printer&view=three-quarter&spin=0&power=0');
  await page.waitForFunction(() => (window as ReviewWindow).__MODEL_REVIEW_DIAGNOSTICS__?.ready === true);
  ...
  await writeFile(
    path.join(OUTPUT_DIR, 'capture-report.json'),
    `${JSON.stringify({ model: 'printer', captures, errors }, null, 2)}\n`,
    'utf8',
  );
