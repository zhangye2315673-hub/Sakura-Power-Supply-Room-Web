...
const OUTPUT_DIR = path.resolve('artifacts/appliance-v2/bubble-machine/evidence/models/bubble-machine');
...
test('captures bubble machine v2 static views and frozen bubble phases', async ({ page }) => {
  ...
  for (const view of VIEWS) {
    await page.goto(`/model-review.html?model=bubble-machine&view=${view}&spin=0&power=0`);
    ...
    expect(diagnostics?.drawCalls ?? 0).toBeGreaterThan(60);
    captures.push({ state: 'idle', view, screenshotPath, diagnostics });
  }

  await page.goto('/model-review.html?model=bubble-machine&view=three-quarter&spin=0&power=0');
  await page.waitForFunction(() => (window as ReviewWindow).__MODEL_REVIEW_DIAGNOSTICS__?.ready === true);
  ...
  await writeFile(path.join(OUTPUT_DIR, 'capture-report.json'), `${JSON.stringify({ model: 'bubble-machine', captures, errors }, null, 2)}\n`);
