...
const OUTPUT_DIR = path.resolve('artifacts/appliance-v2/induction-cooktop/evidence/models/induction-cooktop');
const VIEWS = ['front', 'side', 'back', 'three-quarter'] as const;
const PHASES = [{ id: 'startup', time: 0.72 }, { id: 'climax', time: 3.61 }, { id: 'wind-down', time: 4.75 }] as const;
...
test('captures induction cooktop v2 static views and frozen hotpot phases', async ({ page }) => {
  ...
  for (const view of VIEWS) {
    await page.goto(`/model-review.html?model=induction-cooktop&view=${view}&spin=0&power=0`);
    ...
    expect(diagnostics?.triangles ?? 0).toBeGreaterThan(20_000);
    expect(diagnostics?.drawCalls ?? 0).toBeGreaterThan(80);
    captures.push({ state: 'idle', view, screenshotPath, diagnostics });
  }
  await page.goto('/model-review.html?model=induction-cooktop&view=three-quarter&spin=0&power=0');
  await page.waitForFunction(() => (window as ReviewWindow).__MODEL_REVIEW_DIAGNOSTICS__?.ready === true);
  ...
  await writeFile(path.join(OUTPUT_DIR, 'capture-report.json'), `${JSON.stringify({ model: 'induction-cooktop', captures, errors }, null, 2)}\n`);
