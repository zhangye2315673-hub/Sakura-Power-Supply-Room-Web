...
test('game appliance replacement does not create a visible long frame', async ({ page }) => {
  ...
  const result = await page.evaluate((kind) => new Promise<{
    samples: FrameSample[];
    ...
    const sample = (now: number) => {
      ...
      if (replaced && replacementIndex < 0) replacementIndex = samples.length;
      samples.push({
      ...
      if (replacementIndex >= 0 && samples.length >= replacementIndex + 8) {
        resolve({ samples, replacementIndex, initialKinds, finalKinds });
        return;
      }
      if (now >= timeoutAt) {
  ...
  expect(result.replacementIndex).toBeGreaterThan(20);
  expect(result.finalKinds).not.toEqual(result.initialKinds);
  const steadyFrames = result.samples
    .filter((_, index) => index > 8 && Math.abs(index - result.replacementIndex) > 5)
    .map((sample) => sample.deltaMs);
  ...
  const spawnHandle = await page.waitForFunction(
    (kind) => window.__THREE_GAME_DIAGNOSTICS__?.appliances.find(
      (appliance) => appliance.kind === kind && appliance.state === 'spawning',
    ) ?? null,
    replacementKind,
    { timeout: 5_000 },
  );
  const spawnState = await spawnHandle.jsonValue();
  expect(
    (spawnState?.rootScreenY ?? 1) + (spawnState?.screenHeight ?? 0) * 0.5,
    'the replacement appliance must begin fully above the viewport',
  ).toBeLessThan(-0.08);
  await page.waitForFunction(
    (kind) => {
