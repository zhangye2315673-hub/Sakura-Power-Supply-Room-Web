...
const OUTPUT_DIR = path.resolve('artifacts/appliance-v2/washer/evidence/models/washer');
...
type ReviewWindow = Window & typeof globalThis & {
  __MODEL_REVIEW_DIAGNOSTICS__?: {
    ready?: boolean; triangles?: number; drawCalls?: number;
    performance: { timelineOwners: number };
  };
  __MODEL_REVIEW_SET_POWER__?: (powered: boolean) => void;
  __MODEL_REVIEW_SET_ELAPSED__?: (elapsed: number | null) => void;
};

test('captures washer v2 static views and frozen tumble phases', async ({ page }) => {
  ...
  for (const view of VIEWS) {
    await page.goto(`/model-review.html?model=washer&view=${view}&spin=0&power=0`);
    ...
    expect(diagnostics?.triangles ?? 0).toBeGreaterThan(5_000);
    expect(diagnostics?.drawCalls ?? 0).toBeGreaterThan(30);
    captures.push({ state: 'idle', view, screenshotPath, diagnostics });
  }

  await page.goto('/model-review.html?model=washer&view=three-quarter&spin=0&power=0');
  await page.waitForFunction(() => (window as ReviewWindow).__MODEL_REVIEW_DIAGNOSTICS__?.ready === true);
  ...
  await writeFile(path.join(OUTPUT_DIR, 'capture-report.json'), `${JSON.stringify({ model: 'washer', captures, errors }, null, 2)}\n`);
