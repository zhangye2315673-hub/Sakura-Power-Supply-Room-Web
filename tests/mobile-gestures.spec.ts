...
for (const lantern of [false, true]) test(`touch gestures keep clicks separate with lantern=${lantern}`,  async ({ browser }) => {
  ...
  try {
    await page.route('**/src/main.ts*', route => route.fulfill({ contentType: 'application/javascript', body: `
      ...
      let clicks = 0;
      let viewChanges = 0;
      const orbit = new OrbitController(canvas, new PerspectiveCamera(), { onClick: () => { clicks++; }, onHover: () => {}, onLeave: () => {}, onViewChanged: () => { viewChanges++; } });
      orbit.setTouchLantern(${lantern});
      window.readGesture = () => ({ ...orbit.getState(), clicks, viewChanges });
    ` }));
    await page.goto('/');
    const read = () => page.evaluate(() => (window as unknown as { readGesture: () => { radius: number; yaw: number; clicks: number; viewChanges: number } }).readGesture());
    ...
    const viewsBeforeDrag = (await read()).viewChanges;
    ...
    expect((await read()).clicks).toBe(1);
    await expect.poll(async () => (await read()).viewChanges).toBe(viewsBeforeDrag + 1);
    await page.waitForTimeout(450);
    expect((await read()).viewChanges).toBe(viewsBeforeDrag + 1);
    await page.touchscreen.tap(180, 400);
    expect((await read()).clicks).toBe(2);
    await client.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [{ x: 150, y: 400, id: 1 }] });
    await client.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [{ x: 190, y: 420, id: 1 }] });
    const viewsBeforeCancel = (await read()).viewChanges;
    await client.send('Input.dispatchTouchEvent', { type: 'touchCancel', touchPoints: [] });
    await page.waitForTimeout(450);
    expect((await read()).viewChanges).toBe(viewsBeforeCancel);
    expect((await read()).clicks).toBe(2);
    if (lantern) {
