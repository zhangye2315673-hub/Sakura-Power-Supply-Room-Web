#!/usr/bin/env python3
"""Build deterministic review and delivery boards for microwave v2."""
...
ARTIFACTS = ROOT / "artifacts" / "appliance-v2" / "microwave"
V1 = ROOT / "docs" / "history" / "appliance-model-v1-2026-08-11" / "screenshots" / "models" / "microwave"
REFERENCE = ROOT / "references" / "intake-v2" / "microwave" / "views"
...
def comparison(columns: list[tuple[str, list[Path]]], output: Path, title: str) -> None:
    ...
    for column, (column_label, paths) in enumerate(columns):
        ...
        for row, (view, source_path) in enumerate(zip(VIEWS, paths, strict=True)):
            y = header + row * (tile_size[1] + label_height)
            board.paste(fitted(source_path, tile_size), (x, y))
...
def interaction_board(output: Path) -> None:
    entries = [
        ...
        ("v2 startup", ARTIFACTS / "animation" / "startup" / "render-on-three-quarter.png"),
    ...
    draw = ImageDraw.Draw(board)
    draw.text((margin, 20), "SAKURA microwave v1 / v2 animation contact", fill=INK, font=ImageFont.load_default(size=22))
...
def main() -> None:
    ...
    comparison(
        ...
        ARTIFACTS / "review" / "microwave-v1-v2-four-view-comparison.png",
        "SAKURA microwave - v1 / v2 runtime comparison",
    )
    comparison(
        [("GPT Image 2 reference", concept), ("procedural Three.js v2", v2)],
        ARTIFACTS / "review" / "microwave-reference-v2-four-view-comparison.png",
        "SAKURA microwave - admitted reference / runtime evidence",
    )
    comparison(
        [("v1 runtime", v1), ("GPT Image 2 reference", concept), ("v2 runtime", v2)],
        ARTIFACTS / "final" / "microwave-v2-delivery-board.png",
        "SAKURA microwave v2 delivery board",
    )
    interaction_board(ARTIFACTS / "interaction" / "microwave-animation-comparison.png")
    print(ARTIFACTS / "final" / "microwave-v2-delivery-board.png")
