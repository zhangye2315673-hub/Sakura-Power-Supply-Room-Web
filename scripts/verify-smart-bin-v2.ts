...
import { createSmartBinModel as createV2 } from '../src/appliances/models/smartBin';
import { createSmartBinModel as createV1 } from '../docs/history/appliance-model-v1-2026-08-11/src/appliances/models/smartBin';

const options = { id: 'smart-bin', accent: 0xe8aec4 };
...
function transform(root: THREE.Object3D, name: string) {
...
function frozenNames(root: THREE.Object3D): string[] {
  ...
  root.traverse((object) => {
    if (object !== root && object.name && (object instanceof THREE.Group || object.userData.socket === true)) {
...
function geometry(root: THREE.Object3D) {
  ...
  let unnamedMeshes = 0;
  let finite = true;
  let transparentVisibleOutlines = 0;
  root.traverse((object) => {
    ...
    if (object.userData.isOutline) {
      ...
      const source = object.parent instanceof THREE.Mesh ? object.parent : null;
      const materials = source ? (Array.isArray(source.material) ? source.material : [source.material]) : [];
      if (object.visible && materials.some((material) => material.transparent)) transparentVisibleOutlines += 1;
      return;
    }
    drawMeshes += 1;
    if (object.userData.outlineTier === 'excluded') outlineTiers.excluded += 1;
  ...
  return { triangles: Math.round(triangles), drawMeshes, unnamedMeshes, finite, outlineTiers, transparentVisibleOutlines };
}

function dispose(build: ReturnType<typeof createV2>): number {
  ...
  return [...geometries].filter((geometry) => !geometry.attributes.position).length;
}
...
const drawMeshRatio = v2Geometry.drawMeshes / Math.max(v1Geometry.drawMeshes, 1);
...
if (!v2Geometry.outlineTiers.excluded) failures.push('missing outline exclusions');
if (v2Geometry.transparentVisibleOutlines) failures.push(`${v2Geometry.transparentVisibleOutlines} transparent meshes retain visible outlines`);
if (triangleRatio > 1.35) failures.push(`triangle ratio exceeds 1.35: ${round(triangleRatio)}`);
if (drawMeshRatio > 1.20) failures.push(`draw mesh ratio exceeds 1.20: ${round(drawMeshRatio)}`);
if (new Set(rebuildBounds.map(JSON.stringify)).size !== 1) failures.push('rebuild bounds are not deterministic');
if (runtime.colliders.length !== 3) failures.push(`expected 3 colliders, got ${runtime.colliders.length}`);
...
const report = {
  ...
  drawMeshRatio: round(drawMeshRatio),
...
mkdirSync('artifacts/appliance-v2/smart-bin/diagnostics', { recursive: true });
writeFileSync('artifacts/appliance-v2/smart-bin/diagnostics/smart-bin-v2-verification.json', `${JSON.stringify(report, null, 2)}\n`);
