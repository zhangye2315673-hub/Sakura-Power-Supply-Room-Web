...
import { createWasherModel as createV2 } from '../src/appliances/models/washer';
import { createWasherModel as createV1 } from '../docs/history/appliance-model-v1-2026-08-11/src/appliances/models/washer';

const options = { id: 'washer', accent: 0xe8aec4 };
const frozenPattern = /(?:pivot|socket)$|washer-(?:laundry-volume|volumetric-suds-bubble|volumetric-glass-droplet|foot)-\d+$/;
...
function bounds(root: THREE.Object3D) {
  ...
  return {
    min: box.min.toArray().map(round), max: box.max.toArray().map(round),
...
function frozenNames(root: THREE.Object3D): string[] {
  ...
  root.traverse((object) => {
    if (object !== root && object.name && (object instanceof THREE.Group || frozenPattern.test(object.name))) names.push(object.name);
  });
  return names.sort();
}
...
const report = {
  model: 'washer', v1Bounds, v2Bounds, relativeDelta: relativeDelta.map(round), frozenTransforms,
...
mkdirSync('artifacts/appliance-v2/washer/diagnostics', { recursive: true });
writeFileSync('artifacts/appliance-v2/washer/diagnostics/washer-v2-verification.json', `${JSON.stringify(report, null, 2)}\n`);
