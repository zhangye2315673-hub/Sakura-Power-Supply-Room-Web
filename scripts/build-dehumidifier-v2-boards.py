...
ART = ROOT / 'artifacts' / 'appliance-v2' / 'dehumidifier'
V1 = ROOT / 'docs' / 'history' / 'appliance-model-v1-2026-08-11' / 'screenshots' / 'models' / 'dehumidifier'
V2 = ART / 'evidence' / 'models' / 'dehumidifier'
REF = ROOT / 'references' / 'intake-v2' / 'dehumidifier' / 'views'
...
def interaction(output: Path) -> None:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((margin, 20), 'SAKURA dehumidifier v1 / v2 animation contact', fill=INK, font=ImageFont.load_default(size=21))
...
def main() -> None:
    ...
    board([('v1 runtime', v1), ('admitted existing refs', ref), ('v2 runtime', v2)], ART / 'final' / 'dehumidifier-v2-delivery-board.png', 'SAKURA dehumidifier v2 delivery board')
    board([('admitted existing refs', ref), ('procedural v2', v2)], ART / 'review' / 'dehumidifier-reference-v2-four-view-comparison.png', 'SAKURA dehumidifier reference / runtime')
    interaction(ART / 'interaction' / 'dehumidifier-animation-comparison.png')
    print(ART / 'final' / 'dehumidifier-v2-delivery-board.png')
