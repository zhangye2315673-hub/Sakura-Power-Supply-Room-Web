...
ART = ROOT / 'artifacts' / 'appliance-v2' / 'coffee-maker'
V1 = ROOT / 'docs' / 'history' / 'appliance-model-v1-2026-08-11' / 'screenshots' / 'models' / 'coffee-maker'
V2 = ART / 'evidence' / 'models' / 'coffee-maker'
REF = ROOT / 'references' / 'intake-v2' / 'coffee-maker' / 'views'
...
def board(columns: list[tuple[str, list[Path]]], output: Path, title: str) -> None:
    ...
    canvas = Image.new(
        'RGB',
        (margin * 2 + len(columns) * tile[0], header + len(VIEWS) * (tile[1] + label) + margin),
        BG,
    )
...
def interaction(output: Path) -> None:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((margin, 20), 'SAKURA coffee maker v1 / v2 animation contact', fill=INK, font=ImageFont.load_default(size=21))
...
def main() -> None:
    ...
    board(
        [('v1 runtime', v1), ('GPT Image 2 reference', ref), ('v2 runtime', v2)],
        ART / 'final' / 'coffee-maker-v2-delivery-board.png',
        'SAKURA coffee maker v2 delivery board',
    )
    board(
        [('GPT Image 2 reference', ref), ('procedural v2', v2)],
        ART / 'review' / 'coffee-maker-reference-v2-four-view-comparison.png',
        'SAKURA coffee maker reference / runtime',
    )
    interaction(ART / 'interaction' / 'coffee-maker-animation-comparison.png')
    print(ART / 'final' / 'coffee-maker-v2-delivery-board.png')
