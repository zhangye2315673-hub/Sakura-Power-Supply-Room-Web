...
const outputRoot = path.resolve(process.argv[3] ?? 'artifacts/appliance-v2/microwave/game');
...
await page.waitForFunction(
  (expectedSeed) => {
    ...
    return diagnostics?.seed === expectedSeed
      ...
      && diagnostics.appliances.some((item) => item.id === 'microwave');
  },
  seed,
  { timeout: 150_000 },
);
...
for (const viewport of viewports) {
  ...
  const diagnostics = await page.evaluate(() => {
    ...
    return {
      ...
      target: game.appliances.find((item) => item.id === 'microwave') ?? null,
