...
ART = ROOT / 'artifacts' / 'appliance-v2' / 'printer'
V1 = ROOT / 'docs' / 'history' / 'appliance-model-v1-2026-08-11' / 'screenshots' / 'models' / 'printer'
V2 = ART / 'evidence' / 'models' / 'printer'
REF = ROOT / 'references' / 'intake-v2' / 'printer' / 'views'
...
def interaction(output: Path) -> None:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((margin, 20), 'SAKURA printer v1 / v2 animation contact', fill=INK, font=ImageFont.load_default(size=21))
...
def main() -> None:
    ...
    board([('v1 runtime', v1), ('GPT Image 2 reference', ref), ('v2 runtime', v2)], ART / 'final/printer-v2-delivery-board.png', 'SAKURA printer v2 delivery board')
    board([('GPT Image 2 reference', ref), ('procedural v2', v2)], ART / 'review/printer-reference-v2-four-view-comparison.png', 'SAKURA printer reference / runtime')
    interaction(ART / 'interaction/printer-animation-comparison.png')
    print(ART / 'final/printer-v2-delivery-board.png')
