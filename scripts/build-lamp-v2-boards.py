...
ART = ROOT / 'artifacts' / 'appliance-v2' / 'lamp'
V1 = ROOT / 'docs' / 'history' / 'appliance-model-v1-2026-08-11' / 'screenshots' / 'models' / 'lamp'
V2 = ART / 'evidence' / 'models' / 'lamp'
...
def animation(output: Path) -> None:
    items = [
        ('startup 0.6s', V1 / 'active-startup-0.6.png'),
        ('climax 2.8s', V1 / 'active-climax-2.8.png'),
        ('wind-down 4.75s', V1 / 'active-wind-down-4.75.png'),
    ]
    tile = (390, 293)
    margin, header, label = 24, 70, 28
    canvas = Image.new('RGB', (margin * 2 + 3 * tile[0], header + tile[1] + label + margin), BG)
    draw = ImageDraw.Draw(canvas)
    draw.text((margin, 20), 'SAKURA lamp v2 animation contact', fill=INK, font=ImageFont.load_default(size=21))
    for index, (name, source) in enumerate(items):
        x, y = margin + index * tile[0], header
        canvas.paste(fitted(source, tile), (x, y))
...
board([('v1 archived reference', v1), ('v2 runtime', v2)], ART / 'final' / 'lamp-v2-delivery-board.png', 'SAKURA lamp v2 delivery board')
board([('v1 archived reference', v1), ('v2 runtime', v2)], ART / 'review' / 'lamp-reference-v2-comparison.png', 'SAKURA lamp reference / runtime')
animation(ART / 'final' / 'lamp-v2-animation-board.png')
