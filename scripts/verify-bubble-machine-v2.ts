...
import { createBubbleMachineModel as createV2 } from '../src/appliances/models/bubbleMachine';
import { createBubbleMachineModel as createV1 } from '../docs/history/appliance-model-v1-2026-08-11/src/appliances/models/bubbleMachine';

const options = { id: 'bubble-machine', accent: 0xe8aec4 };
const frozenPattern = /(?:pivot|socket|rig)$|bubble-machine-(?:performance-bubble|giant-bubble|burst-|foot)-\d+$/;
...
function frozenNames(root: THREE.Object3D): string[] {
  ...
  root.traverse((object) => {
    if (object !== root && object.name && (object instanceof THREE.Group || object.userData.socket === true || object.userData.performanceEffect === true || frozenPattern.test(object.name))) names.push(object.name);
  });
...
for (const [name, matches] of Object.entries(frozenTransforms)) {
  ...
  const object = v1.root.getObjectByName(name);
  if ((object?.userData.socket === true || (object instanceof THREE.Group && object.userData.performanceEffect !== true)) && !runtime.nodes[name]) failures.push(`runtime node missing: ${name}`);
}
...
const report = { model: 'bubble-machine', v1Bounds, v2Bounds, relativeDelta: relativeDelta.map(round), groundDelta: round(groundDelta), frozenNodeCount: frozen.length, frozenTransforms, v1Geometry, v2Geometry, triangleRatio: round(triangleRatio), colliders: runtime.colliders.length, destructionGroups: runtime.destructionGroups.length, rebuildBounds, failures, passed: failures.length === 0 };
mkdirSync('artifacts/appliance-v2/bubble-machine/diagnostics', { recursive: true });
writeFileSync('artifacts/appliance-v2/bubble-machine/diagnostics/bubble-machine-v2-verification.json', `${JSON.stringify(report, null, 2)}\n`);
