...
const outputRoot = path.resolve(process.argv[3] ?? 'artifacts/img2threejs-v2/game-controller/game');
const seed = 2;
const viewports = [
...
await page.goto(`${baseUrl}/?seed=${seed}&mode=random&direct=1`, { waitUntil: 'domcontentloaded' });
...
await page.waitForFunction(
  (expectedSeed) => {
    ...
    return diagnostics?.seed === expectedSeed
      ...
      && diagnostics.appliances.some((item) => item.id === 'game-controller');
  },
  seed,
  { timeout: 150_000 },
);
...
for (const viewport of viewports) {
  ...
  const diagnostics = await page.evaluate(() => {
    ...
    return {
      ...
      target: game.appliances.find((item) => item.id === 'game-controller') ?? null,
      clickTarget: game.clickTarget,
      blockedClickTarget: game.blockedClickTarget,
  ...
  const targetCenterInsideViewport = Boolean(
    ...
    && target.screenX >= 0 && target.screenX <= 1
    && target.screenY >= 0 && target.screenY <= 1,
  );
  ...
  captures.push({ seed, viewport, fullPagePath, canvasPath, targetCenterInsideViewport, targetEnvelopeInsideViewport, diagnostics });
}
...
const report = {
  ...
  passed: consoleErrors.length === 0
    && pageErrors.length === 0
    && captures.every((capture) => capture.targetCenterInsideViewport && capture.targetEnvelopeInsideViewport),
};
...
console.log(JSON.stringify({
  ...
  captures: captures.map((capture) => ({
    ...
    target: capture.diagnostics.target,
