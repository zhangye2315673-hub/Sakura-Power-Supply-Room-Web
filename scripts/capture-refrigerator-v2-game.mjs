...
const outputRoot = path.resolve(process.argv[3] ?? 'artifacts/appliance-v2/refrigerator/game');
const seed = 2;
const viewports = [
...
await page.waitForFunction(
  (expectedSeed) => {
    ...
    return diagnostics?.seed === expectedSeed
      ...
      && diagnostics.appliances.some((item) => item.id === 'refrigerator');
  },
  seed,
  { timeout: 150_000 },
);
...
for (const viewport of viewports) {
  ...
  const diagnostics = await page.evaluate(() => {
    ...
    return {
      ...
      target: game.appliances.find((item) => item.id === 'refrigerator') ?? null,
