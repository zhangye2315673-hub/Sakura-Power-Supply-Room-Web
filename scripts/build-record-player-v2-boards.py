...
ART = ROOT / 'artifacts' / 'appliance-v2' / 'record-player'
V1 = ROOT / 'docs' / 'history' / 'appliance-model-v1-2026-08-11' / 'screenshots' / 'models' / 'record-player'
V2 = ART / 'evidence' / 'models' / 'record-player'
REF = ROOT / 'references' / 'intake-v2' / 'record-player' / 'views'
...
def animation(output: Path) -> None:
    items = [('startup 0.6s', V2 / 'render-startup-three-quarter.png'), ('climax 2.8s', V2 / 'render-climax-three-quarter.png'), ('wind-down 4.75s', V2 / 'render-wind-down-three-quarter.png')]
    tile = (390, 293); margin = 24; header = 70; label = 28; canvas = Image.new('RGB', (margin * 2 + 3 * tile[0], header + tile[1] + label + margin), BG); draw = ImageDraw.Draw(canvas); draw.text((margin, 20), 'SAKURA record player v2 playback contact', fill=INK, font=ImageFont.load_default(size=21))
...
board([('v1 runtime', v1), ('procedural v2', v2)], ART / 'final' / 'record-player-v2-delivery-board.png', 'SAKURA record player v1 / v2 delivery board')
board([('conditional v1 fallback', ref), ('procedural v2', v2)], ART / 'review' / 'record-player-reference-v2-four-view-comparison.png', 'SAKURA record player reference boundary / runtime')
animation(ART / 'final' / 'record-player-v2-animation-board.png')
print(ART / 'final' / 'record-player-v2-delivery-board.png')
