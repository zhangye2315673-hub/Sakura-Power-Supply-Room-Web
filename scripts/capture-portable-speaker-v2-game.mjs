...
const outputRoot = path.resolve(process.argv[3] ?? 'artifacts/appliance-v2/portable-speaker/game');
...
await page.waitForFunction(
  (expectedSeed) => {
    ...
    return diagnostics?.seed === expectedSeed
      ...
      && diagnostics.appliances.some((item) => item.id === 'portable-speaker');
  },
  seed,
  { timeout: 150_000 },
);
...
for (const viewport of viewports) {
  ...
  const diagnostics = await page.evaluate(() => {
    ...
    return {
      ...
      target: game.appliances.find((item) => item.id === 'portable-speaker') ?? null,
  ...
  captures.push({ viewport, fullPagePath, canvasPath, targetCenterInsideViewport, targetEnvelopeInsideViewport, diagnostics });
}
