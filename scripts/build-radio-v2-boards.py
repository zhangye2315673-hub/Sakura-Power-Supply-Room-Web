...
ART = ROOT / 'artifacts' / 'appliance-v2' / 'radio'
REF = ROOT / 'references' / 'intake-v2' / 'radio' / 'views'
V2 = ART / 'evidence' / 'models' / 'radio'
...
def interaction(output: Path) -> None:
    ...
    draw = ImageDraw.Draw(canvas); draw.text((margin, 20), 'SAKURA radio v2 animation contact', fill=INK, font=ImageFont.load_default(size=21))
...
board([('conditional v1 fallback', ref), ('procedural v2', v2)], ART / 'review' / 'radio-reference-v2-four-view-comparison.png', 'SAKURA radio reference / runtime')
board([('conditional v1 fallback', ref), ('v2 runtime', v2)], ART / 'final' / 'radio-v2-delivery-board.png', 'SAKURA radio v2 delivery board')
interaction(ART / 'final' / 'radio-v2-animation-board.png')
print(ART / 'final' / 'radio-v2-delivery-board.png')
