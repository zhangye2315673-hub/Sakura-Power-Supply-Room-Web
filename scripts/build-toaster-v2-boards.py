...
ART = ROOT / 'artifacts' / 'appliance-v2' / 'toaster'
V1 = ROOT / 'docs' / 'history' / 'appliance-model-v1-2026-08-11' / 'screenshots' / 'models' / 'toaster'
V2 = ART / 'evidence' / 'models' / 'toaster'
REF = ROOT / 'references' / 'intake-v2' / 'toaster' / 'views'
...
def fitted(path: Path, size: tuple[int, int]) -> Image.Image:
    ...
    image.thumbnail(size, Image.Resampling.LANCZOS); tile = Image.new('RGB', size, BG)
    tile.paste(image, ((size[0] - image.width) // 2, (size[1] - image.height) // 2)); return tile
...
def interaction(output: Path) -> None:
    ...
    draw.text((margin, 20), 'SAKURA toaster v1 / v2 animation contact', fill=INK, font=ImageFont.load_default(size=21))
...
def main() -> None:
    ...
    board([('v1 runtime', v1), ('GPT Image 2 reference', ref), ('v2 runtime', v2)], ART / 'final/toaster-v2-delivery-board.png', 'SAKURA toaster v2 delivery board')
    board([('GPT Image 2 reference', ref), ('procedural v2', v2)], ART / 'review/toaster-reference-v2-four-view-comparison.png', 'SAKURA toaster reference / runtime')
    interaction(ART / 'interaction/toaster-animation-comparison.png'); print(ART / 'final/toaster-v2-delivery-board.png')
