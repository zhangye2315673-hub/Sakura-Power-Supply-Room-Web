...
ART = ROOT / 'artifacts' / 'appliance-v2' / 'gumball-machine'
V1 = ROOT / 'docs' / 'history' / 'appliance-model-v1-2026-08-11' / 'screenshots' / 'models' / 'gumball-machine'
V2 = ART / 'evidence' / 'models' / 'gumball-machine'
REF = ROOT / 'references' / 'intake-v2' / 'gumball-machine' / 'views'
...
def interaction(output: Path) -> None:
    ...
    items = [(f'v1 {name}', V1 / f'active-{name}-{time}.png') for name, time in phases] + [(f'v2 {name}', V2 / f'active-{name}-{time}.png') for name, time in phases]
    tile = (390, 293); margin = 24; header = 70; label = 28
    canvas = Image.new('RGB', (margin * 2 + 3 * tile[0], header + 2 * (tile[1] + label) + margin), BG); draw = ImageDraw.Draw(canvas)
    draw.text((margin, 20), 'SAKURA gumball machine v1 / v2 animation contact', fill=INK, font=ImageFont.load_default(size=21))
...
def animation_board(output: Path) -> None:
    ...
    draw = ImageDraw.Draw(canvas); draw.text((margin, 20), 'SAKURA gumball machine frozen animation alignment', fill=INK, font=ImageFont.load_default(size=21))
...
def main() -> None:
    ...
    board([('v1 runtime', v1), ('GPT Image 2 reference', ref), ('v2 runtime', v2)], ART / 'final' / 'gumball-machine-v2-delivery-board.png', 'SAKURA gumball machine v2 delivery board')
    board([('GPT Image 2 reference', ref), ('procedural v2', v2)], ART / 'review' / 'gumball-machine-reference-v2-four-view-comparison.png', 'SAKURA gumball machine reference / runtime')
    interaction(ART / 'interaction' / 'gumball-machine-animation-comparison.png')
    animation_board(ART / 'final' / 'gumball-machine-v2-animation-board.png')
    print(ART / 'final' / 'gumball-machine-v2-delivery-board.png')
