...
ART = ROOT / 'artifacts' / 'appliance-v2' / 'humidifier'
V1 = ROOT / 'docs' / 'history' / 'appliance-model-v1-2026-08-11' / 'screenshots' / 'models' / 'humidifier'
V2 = ART / 'evidence' / 'models' / 'humidifier'
REF = ROOT / 'references' / 'intake-v2' / 'humidifier' / 'views'
...
def animation(output: Path) -> None:
    ...
    tile = (390, 293); margin = 24; header = 70; label = 28; canvas = Image.new('RGB', (margin * 2 + 3 * tile[0], header + tile[1] + label + margin), BG); draw = ImageDraw.Draw(canvas); draw.text((margin, 20), 'SAKURA humidifier v2 weather contact', fill=INK, font=ImageFont.load_default(size=21))
...
board([('v1 runtime', v1), ('procedural v2', v2)], ART / 'final' / 'humidifier-v2-delivery-board.png', 'SAKURA humidifier v1 / v2 delivery board')
board([('conditional v1 fallback', ref), ('procedural v2', v2)], ART / 'review' / 'humidifier-reference-v2-four-view-comparison.png', 'SAKURA humidifier reference / runtime')
animation(ART / 'final' / 'humidifier-v2-animation-board.png')
print(ART / 'final' / 'humidifier-v2-delivery-board.png')
