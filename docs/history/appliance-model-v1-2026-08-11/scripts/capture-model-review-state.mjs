...
for (const view of viewsArg.split(',')) {
  await page.goto(`${baseUrl}?model=${encodeURIComponent(model)}&view=${view}&spin=0&power=0`, { waitUntil: 'domcontentloaded' });
...
await browser.close();
console.log(JSON.stringify({ model, state, captures }, null, 2));
