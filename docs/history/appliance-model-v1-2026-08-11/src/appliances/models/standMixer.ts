...
const REFERENCE_PATH = 'references/intake/stand-mixer/front.png';
...
export function createStandMixerModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  for (let i = 0; i < 6; i += 1) {
    const wire = kit.mesh(`stand-mixer-whisk-wire-${i + 1}`, new THREE.TubeGeometry(whiskWireCurve(i * Math.PI / 3), 18, 0.026, 6, false), steel, beaterPivot, false);
    wire.position.y = -0.4;
  ...
  [Math.PI * 0.75, Math.PI * 1.75].forEach((angle, i) => {
    const handle = kit.mesh(`stand-mixer-bowl-handle-${i + 1}`, new THREE.TubeGeometry(handleCurve(angle), 24, 0.095, 8, false), creamHighlight, bowlPivot);
