...
const REFERENCE_PATH = 'E:/AI/codexAI/sakula/arrow-cube/references/intake/dehumidifier/front.png';
...
function makeRoundedRectangleFrame(
  ...
  const outer = roundedRectangleRing(outerWidth, outerDepth, outerRadius, 8);
  const inner = roundedRectangleRing(innerWidth, innerDepth, innerRadius, 8).reverse();
...
export function createDehumidifierModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  (levelGlint.material as THREE.Material).transparent = true;
  (levelGlint.material as THREE.Material).opacity = 0.48;
  ...
  const controlRing = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.2, 0.2, 0.07, 32),
    pinkShadow,
    controlPivot,
  );
  ...
  const controlFace = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.164, 0.164, 0.075, 32),
    pinkEdge,
    controlPivot,
  );
  ...
  const controlGlint = kit.mesh(
    ...
    new THREE.TorusGeometry(0.154, 0.006, 5, 32),
    creamLight,
    controlPivot,
    false,
  );
