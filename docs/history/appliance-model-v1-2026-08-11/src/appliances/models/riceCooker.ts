...
export function createRiceCookerModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const releaseSeat = kit.mesh(
    ...
    new RoundedBoxGeometry(0.36, 0.18, 0.055, 4, 0.025),
    accentDeep,
    lidPivot,
    false,
  );
  ...
  const panel = kit.mesh(
    ...
    new THREE.CapsuleGeometry(0.19, 0.3, 7, 16),
    accentSoft,
    bodyPivot,
  );
  ...
  const leftLamp = kit.mesh(
    ...
    new THREE.SphereGeometry(0.046, 10, 7),
  ...
  const rearHinge = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.1, 0.1, 0.58, 14),
    accentDeep,
    bodyPivot,
  );
  ...
  for (const x of [-0.32, 0.32]) {
    const collar = kit.mesh(
      ...
      new THREE.CylinderGeometry(0.13, 0.13, 0.12, 14),
      accent,
      bodyPivot,
    );
