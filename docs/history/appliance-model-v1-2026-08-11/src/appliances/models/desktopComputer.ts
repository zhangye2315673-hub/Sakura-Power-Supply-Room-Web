...
const REFERENCE_PATH = 'references/intake/desktop-computer/front.png';
...
export function createDesktopComputerModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const pink = shifted(options.accent, 0.015);
  ...
  const creamHighlight = kit.material(0xfff6e5, { tint: 0x867987 });
  ...
  const screenOff = kit.material(0x39383e, { tint: 0x272631, emissive: 0x000000 });
  ...
  const crtBell = kit.mesh('desktop-computer-crt-tapered-rear-bell', rounded(4.38, 2.66, 1.42, 0.36), cream, screenTiltPivot);
  ...
  const topBezel = kit.mesh('desktop-computer-monitor-bezel-top', rounded(5.45, 0.28, bezelDepth, 0.1), creamHighlight, screenTiltPivot);
  topBezel.position.set(0, 1.56, 0.29);
  topBezel.userData.part = 'monitor-bezel';
  const bottomBezel = kit.mesh('desktop-computer-monitor-bezel-bottom', rounded(5.45, 0.31, bezelDepth, 0.1), creamHighlight, screenTiltPivot, false);
  bottomBezel.position.set(0, -1.54, 0.29);
  bottomBezel.userData.explodeWithParent = true;
  [-1, 1].forEach((side, i) => {
    const rail = kit.mesh(`desktop-computer-monitor-bezel-side-${i + 1}`, rounded(0.28, 3.08, bezelDepth, 0.1), creamHighlight, screenTiltPivot, false);
  ...
  const topCap = kit.mesh('desktop-computer-pink-tower-top-cap', rounded(2.75, 0.38, 3.04, 0.2), pinkMaterial, towerPivot);
  ...
  const keyboardShell = kit.mesh('desktop-computer-rounded-keyboard-shell', rounded(5.35, 0.54, 1.72, 0.18), cream, keyboardPivot);
  keyboardShell.position.y = 0.34;
  ...
  const mouseTop = kit.mesh('desktop-computer-pink-mouse-upper-shell', new THREE.SphereGeometry(0.75, 28, 16, 0, Math.PI * 2, 0, Math.PI * 0.52), pinkMaterial, mousePivot);
  ...
  [-1, 1].forEach((side, i) => {
    const button = kit.mesh(`desktop-computer-mouse-button-${i + 1}`, rounded(0.52, 0.1, 0.63, 0.08), pinkLightMaterial, mouseButtonPivot, false);
  ...
  const mouseWheel = kit.mesh('desktop-computer-mouse-wheel', new THREE.CylinderGeometry(0.09, 0.09, 0.15, 16), cavity, mouseWheelPivot, false);
