...
export function createMicrowaveModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const digitMaterial = kit.material(accentLight, {
    tint: 0x675a70,
    emissive: accentLight,
  });
  ...
  const cabinet = kit.mesh(
    ...
    new RoundedBoxGeometry(1.24, 2.06, 1.58, 5, 0.18),
    shellMaterial,
  );
  ...
  const cabinetTopBridge = kit.mesh(
    ...
    new RoundedBoxGeometry(2.34, 0.23, 1.58, 4, 0.11),
    shellMaterial,
  );
  ...
  const cabinetBottomBridge = kit.mesh(
    ...
    new RoundedBoxGeometry(2.34, 0.23, 1.58, 4, 0.1),
    shellMaterial,
  );
  ...
  const cabinetLeftWall = kit.mesh(
    ...
    new RoundedBoxGeometry(0.28, 1.72, 1.58, 4, 0.1),
    shellMaterial,
  );
  ...
  const lowerRail = kit.mesh(
    ...
    new RoundedBoxGeometry(3.35, 0.17, 1.38, 3, 0.06),
    shellShadowMaterial,
  );
  ...
  const controlPanel = kit.mesh(
    ...
    new RoundedBoxGeometry(0.68, 1.68, 0.1, 4, 0.11),
    accentMaterial,
  );
  ...
  const display = kit.mesh(
    ...
    new RoundedBoxGeometry(0.4, 0.22, 0.035, 2, 0.04),
    displayMaterial,
  );
  ...
  const dialOuter = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.225, 0.225, 0.065, 20),
    accentDarkMaterial,
    dialPivot,
  );
  ...
  const dial = kit.mesh(
    ...
    new THREE.CylinderGeometry(0.185, 0.195, 0.095, 20),
    shellHighlightMaterial,
    dialPivot,
  );
  ...
  for (let index = 0; index < 8; index += 1) {
    ...
    mark.position.set(1.25 + Math.sin(angle) * 0.27, 1.14 + Math.cos(angle) * 0.27, 1.015);
  ...
  const rearPanel = kit.mesh(
    ...
    new RoundedBoxGeometry(3.03, 1.6, 0.055, 4, 0.13),
    shellHighlightMaterial,
  );
  ...
  const ventPanel = kit.mesh(
    ...
    new RoundedBoxGeometry(2.14, 0.86, 0.035, 3, 0.1),
    shellShadowMaterial,
  );
  ...
  const inletPlate = kit.mesh(
    ...
    new RoundedBoxGeometry(0.48, 0.3, 0.035, 2, 0.055),
    accentMaterial,
  );
  inletPlate.position.set(0.9, 0.61, -0.86);
  const inlet = kit.mesh(
    'microwave-rear-power-inlet',
    new RoundedBoxGeometry(0.29, 0.17, 0.045, 2, 0.035),
    cavityMaterial,
  );
  ...
  const build = kit.finish(
    {
      ...
      reconstructed: [
        'wide rounded cabinet with deep side shell, separate front fascia, lower rail and four feet',
        'left-hinged layered door with preserved red perimeter, black inner border, enlarged clear window, restrained safety mesh and tall ivory handle',
        'right control column with display, round button, indexed rotary dial and lower rectangular button',
        'rear recessed service panel with large vent field, four corner fasteners and recessed power inlet',
        ...
        'independent door hinge, Y-axis tray rotor, controls and named sockets for animation and interaction',
      ],
      inferred: [
