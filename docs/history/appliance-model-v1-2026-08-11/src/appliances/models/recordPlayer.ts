...
export function createRecordPlayerModel(options: ApplianceModelOptions): ApplianceModelBuild {
  ...
  const body = kit.mesh('record-player-rounded-body-shell', rounded(3.55, 0.78, 2.5, 0.18), cream, bodyPivot);
  ...
  const platter = kit.mesh('record-player-black-vinyl-platter', new THREE.CylinderGeometry(1.03, 1.03, 0.075, 64), vinyl, platterPivot);
  platter.position.y = 0.06;
  platter.userData.part = 'platter';
  const platterRim = kit.mesh('record-player-pink-platter-perimeter', new THREE.TorusGeometry(1.01, 0.06, 10, 64), pink, platterPivot, false);
  platterRim.rotation.x = Math.PI * 0.5;
  platterRim.position.y = 0.1;
  platterRim.userData.explodeWithParent = true;
  const recordLabel = kit.mesh('record-player-record-label-ring', new THREE.CylinderGeometry(0.34, 0.34, 0.018, 40), cavity, platterPivot, false);
  recordLabel.position.y = 0.102;
  recordLabel.userData.explodeWithParent = true;
  const labelRing = kit.mesh('record-player-record-label-rim', new THREE.TorusGeometry(0.28, 0.015, 7, 32), metal, platterPivot, false);
  ...
  const tonearmBase = kit.mesh('record-player-tonearm-base', new THREE.CylinderGeometry(0.18, 0.2, 0.16, 20), pinkDark, tonearmPivot);
  ...
  addCylinderBetween(kit, 'record-player-tonearm-rise', armStart, armMid, 0.055, cream, tonearmCuePivot, 12);
  addCylinderBetween(kit, 'record-player-tonearm-sweep', armMid, armEnd, 0.045, creamLight, tonearmCuePivot, 12);
  const cartridge = kit.mesh('record-player-pink-cartridge', rounded(0.26, 0.13, 0.18, 0.035), pink, tonearmCuePivot);
  ...
  const knob = kit.mesh('record-player-speed-control-knob', new THREE.CylinderGeometry(0.16, 0.16, 0.13, 20), pink, controlPivot);
