...
const SPAWN_DROP_TOTAL_DURATION = 1.55;
...
const SPAWN_DROP_HEIGHT = 0.46;
const SPAWN_DROP_FALL_DURATION = 0.48;
...
export class ApplianceTarget {
  ...
  beginSpawnDrop(): void {
    ...
    this.dropLanded = false;
    this.dropOffset = -SPAWN_DROP_HEIGHT;
    this.dropVelocity = 0;
...
export class ApplianceScene {
  ...
  private readonly pendingSpawns: Array<{ target: ApplianceTarget; readyAt: number }> = [];
  ...
  update(delta: number, elapsed: number): void {
    ...
    for (let index = this.pendingSpawns.length - 1; index >= 0; index -= 1) {
      ...
      if (now < pending.readyAt) continue;
      this.pendingSpawns.splice(index, 1);
      pending.target.beginSpawnDrop();
  ...
  private updateDrop(target: ApplianceTarget, delta: number): void {
    ...
    if (target.isSpawnDrop) {
      if (elapsed < SPAWN_DROP_FALL_DURATION) {
        ...
        target.dropOffset = -SPAWN_DROP_HEIGHT * (1 - fall * fall);
        return;
      }
  ...
  private replaceTarget(
    ...
    const available = APPLIANCE_CATALOG.filter(
      (definition) => definition.id !== previous.kind && !occupiedKinds.has(definition.id),
    );
    ...
    const definition = pool[Math.floor(this.nextReplacementRandom() * pool.length)] ?? previous.definition;
    this.replacementKindHistory.set(
      targetIndex,
      [...new Set<ApplianceKind>([definition.id, previous.kind, ...recentKinds])]
    ...
    const accentPool = canChangeAccent
      ...
      : [];
    const accent = accentPool[Math.floor(this.nextReplacementRandom() * accentPool.length)]
      ?? previous.accent;

    const replacement = new ApplianceTarget(
      definition,
      accent,
    ...
    previous.dispose();
    this.pendingSpawns.push({ target: replacement, readyAt: now + REPLACEMENT_DELAY });
  }
  ...
  private readonly onPointerDown = (event: PointerEvent) => {
    if (event.button !== 0 || this.pointerId !== null || !this.canvas || !this.camera) return;
