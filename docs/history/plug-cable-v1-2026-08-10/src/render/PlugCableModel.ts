...
const segmentGeometry = new THREE.CylinderGeometry(ARROW_RADIUS, ARROW_RADIUS, 1, 10, 1, false);
const jointGeometry = new THREE.SphereGeometry(ARROW_RADIUS * 1.055, 8, 6);
...
export class PlugCableModel {
  ...
  private readonly tailHead: PlugHead | null;
  private readonly headAvailableHint = createAvailableEndHint();
  private readonly tailAvailableHint: THREE.Group | null;
  ...
  constructor(
    ...
    this.head.shell.userData.arrowId = definition.id;
    this.head.shell.userData.cableEnd = 'head';
    this.tailHead = definition.doubleEnded
      ? createPlugHead(definition.color, PLUG_HEAD_SCALE, false, plugStyleId)
      : null;
    if (this.tailHead) {
      this.tailHead.shell.userData.arrowId = definition.id;
      this.tailHead.shell.userData.cableEnd = 'tail';
    }
  ...
  setHovered(hovered: boolean, end?: CableEnd): void {
    ...
    this.head.setHovered(hovered && (!end || end === 'head'));
    this.tailHead?.setHovered(hovered && (!end || end === 'tail'));
  }
  ...
  private build(points: readonly THREE.Vector3[], withOutline = true, activeEnd: CableEnd = 'head'): void {
    ...
    if (!this.definition.doubleEnded) {
      parts.push(
        transformedGeometry(
          tailGeometry,
    ...
    this.bodyMesh = mesh;
    if (this.tailHead) this.pickMeshes.push(this.head.shell, this.tailHead.shell);
    else this.pickMeshes.push(mesh, this.head.shell);
    this.root.add(mesh);
    ...
    activeHead?.root.quaternion.setFromUnitVectors(
      up,
      activeEnd === 'tail' ? this.tailExitDirection : this.exitDirection,
    );
    if (passiveHead) {
