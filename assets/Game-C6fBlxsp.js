...
var MusicPlayer = class {
	...
	tick() {
		const now = performance.now();
		const step = Math.min(.1, (now - this.lastTick) / 1e3) * .16 / 1.2;
		this.lastTick = now;
		let fading = false;
		this.players.forEach((player, profile) => {
			const target = profile === this.profile ? .16 : 0;
...
var AudioManager = class {
	...
	playInteraction(sound) {
		...
		const gainScale = sound === "blocked" || sound === "failed" || sound === "rush-warning" ? .09 : sound === "rush-tick" ? .04 : .065;
		frequencies[sound].forEach((frequency, index) => {
			this.scheduleTone(this.buses.interaction, frequency, this.context.currentTime + index * .09, sound === "cable-success" ? .28 : sound === "cable-grab" ? .2 : sound === "button" ? .14 : .13, gainScale, "sine");
		});
	}
	playSample(sound) {
		...
		element.volume = sound === "socket-near" ? .12 : sound === "button" ? .18 : .34;
