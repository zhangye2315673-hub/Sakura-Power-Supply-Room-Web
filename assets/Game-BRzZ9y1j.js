...
import { t as ThemeController } from "./game-DTYSJPL8.js";
import { a as renderSkillCardSymbol, c as pickPrinterCopyCableId, i as getSkillCardStyle, n as getApplianceCardLabel, o as APPLIANCE_SKILL_REGISTRY, s as SkillChallengeEngine, t as GACHA_TIER_LABELS } from "./SkillCardPresentation-BKsci2Gv.js";
import { i as selectCampaignAppliances, t as APPLIANCE_CATALOG } from "./ApplianceCatalog-B_KJFoGh.js";
import { T as installJellyEnvironment, _ as POWERED_ACTIVE_DURATION, b as poweredAnimationState, i as RoundedBoxGeometry, l as PORTABLE_SPEAKER_ACTIVE_DURATION, t as mergeGeometries, u as PORTABLE_SPEAKER_BASS_BEATS, v as PRINTER_POWERED_ACTIVE_DURATION, w as cableJelly, x as poweredPreviewCycleDuration } from "./BufferGeometryUtils-CtH-hlZf.js";
import { C as OrbitControls, S as createLampVolumetricBeam, a as createSummerLeafGeometry, b as LAMP_BEAM_FAR_TO_NEAR_RATIO, c as SEASON_PROFILES, d as lerpEnvironmentState, f as AppliancePerformanceSystem, g as TELEVISION_RAPID_SWITCH_TIMES, h as TELEVISION_INITIAL_SWITCHES, i as createSeasonParticleMaterial, l as cloneEnvironmentState, m as createSakuraPetalMaterial, n as createAutumnLeafGeometry, o as createWinterSnowGeometry, p as createSakuraPetalGeometry, r as createFireflyGeometry, s as SEASON_MODES, t as PetalField, u as isSeasonMode, x as LAMP_BEAM_SOURCE_RADIUS_LOCAL, y as LAMP_BEAM_DEFAULT_LENGTH_LOCAL } from "./PetalField-BZccXHdn.js";
import { n as cel, t as SkillPresentationController } from "./SkillPresentationController-Cu0MYo2a.js";
import { a as setHullOutlineStyle, c as markSharedResource, n as setHullOutlineReveal, o as setOutlineResolution, r as setHullOutlineRootFade, s as disposeOwnedResource, t as addHullOutline } from "./outline-Cv1Jem2g.js";
import { A as setCableSkillRecolor, B as PLUG_HEAD_MAX_LENGTH, C as createCableToonMaterial, D as setCableFreeze, E as setCableCoffeeStain, F as DIRECTION_VECTORS, G as STATIC_PLUG_CLEARANCE, H as PLUG_HEAD_PIN_CLEARANCE, I as EXIT_DISTANCE, J as CooperativeYieldBudget, K as cableSocketPointsToWorld, L as LANE_PITCH, M as ARROW_RADIUS, N as CABLE_TAIL_TERMINAL_LENGTH, O as setCableIceShell, P as COLLISION_RADIUS, R as PLUG_HEAD_BODY_CLEARANCE_LENGTH, S as createCableIceShellMaterial, T as createRoundedIceShellGeometry, U as PLUG_HEAD_PIN_RADIUS, V as PLUG_HEAD_MAX_RADIUS, W as STATIC_BODY_CLEARANCE, _ as COFFEE_STAIN_COLOR, a as jellyDefaults, b as REFRIGERATOR_ICE_OPACITY, c as setJellyDynamics, d as applianceBodyBounds, f as PLUG_HEAD_ENVELOPE, g as CABLE_RADIUS, h as createPlugHead, i as getJellyFeel, j as setCableSkillSweep, k as setCableOverheat, l as setJellyFeel, o as jellyDynamics, q as directionKeyFromDelta, r as SoftDeformController, s as saveJellyDynamics, t as ApplianceScene, u as createApplianceModel, v as REFRIGERATOR_FREEZE_COLOR, w as createRoundedCableGeometry, x as REFRIGERATOR_SPIKE_OPACITY, y as REFRIGERATOR_ICE_COLOR, z as PLUG_HEAD_CLEARANCE } from "./ApplianceScene-0iT7RH1c.js";
//#region src/game/CampaignProgress.ts
var CAMPAIGN_STATE_KEY = "plug-spirits-campaign-state-v1";
var campaignStars = (mistakes) => mistakes === 0 ? 3 : mistakes <= 2 ? 2 : 1;
var campaignTime = (ms) => {
...
var CampaignProgress = class {
	...
	constructor(count, storage) {
		...
		try {
			...
			if (state) {
				...
				if (Number.isInteger(legacy) && legacy >= 0 && legacy <= count) for (let id = 1; id <= legacy; id++) this.completed.add(id);
			}
		} catch {}
	...
	readResults(input) {
		...
		for (const [key, value] of Object.entries(input)) {
			...
			const result = value;
			if (!Number.isFinite(result.timeMs) || result.timeMs < 0 || !Number.isInteger(result.mistakes) || result.mistakes < 0 || result.stars !== campaignStars(result.mistakes)) continue;
			this.best[key] = {
	...
	snapshot() {
		...
		const replayLevel = [...completed].sort((a, b) => (this.best[a]?.stars ?? 0) - (this.best[b]?.stars ?? 0) || a - b)[0] ?? 1;
		return {
			completed,
			best: structuredClone(this.best),
			...
			replayLevel,
			allComplete: completed.length === this.count
		};
	}
	record(id, result) {
		...
		const isBetter = !previous || result.stars > previous.stars || result.stars === previous.stars && result.mistakes < previous.mistakes || result.stars === previous.stars && result.mistakes === previous.mistakes && result.timeMs < previous.timeMs;
		const status = !previous ? "first" : isBetter ? "record" : "unchanged";
		this.completed.add(id);
		if (isBetter) this.best[id] = { ...result };
		let persisted = false;
		try {
			this.storage?.setItem(CAMPAIGN_STATE_KEY, JSON.stringify({
		...
		return {
			...
			status,
			persisted,
			unlocked: nextWasLocked ? id + 1 : void 0,
...
function campaignChapterText(id) {
	return {
		name: t(`campaign.chapter.${id}.name`),
		goal: t(`campaign.chapter.${id}.goal`)
	};
}
//#endregion
//#region src/systems/CampaignUi.ts
var CampaignUi = class {
	progress;
	levels;
	goal = document.createElement("details");
	dialog = document.createElement("dialog");
	currentId = null;
	completion = null;
	events = new AbortController();
	homeProgress = document.querySelector("#start-campaign-progress");
	hudProgress = document.querySelector("#campaign-hud-progress");
	summary = document.querySelector("#complete-campaign-summary");
	retry = document.createElement("button");
	improve = document.createElement("button");
	constructor(progress, levels, onSelect, onRetry) {
      ...
      <p id="campaign-dialog-progress"></p><div id="campaign-level-list"></div>`;
		...
		this.dialog.addEventListener("click", (event) => {
			const button = event.target.closest("[data-campaign-level]");
	...
	refresh() {
		...
		const params = {
			done: snapshot.completed.length,
			count,
		...
		if (this.currentId !== null) {
			...
			this.hudProgress.textContent = t("campaign.level", {
				level: level.id,
				count,
				shape: text.name
			});
			this.goal.querySelector("summary").textContent = t("campaign.brief.title", {
				level: level.id,
				name: text.name
			});
			this.goal.querySelector("span").textContent = t("campaign.brief.meta", {
				shape: shapeLabel(level.shape),
		...
		this.dialog.querySelector("#campaign-level-list").replaceChildren(...this.levels.map((level) => {
			...
			state.textContent = button.disabled ? t("campaign.locked", { level: level.id - 1 }) : [done ? t("campaign.done") : t("campaign.unlocked"), current ? t("campaign.current") : ""].filter(Boolean).join(" · ");
			const rating = document.createElement("small");
			const best = snapshot.best[level.id];
			rating.textContent = best ? `${"★".repeat(best.stars)}${"☆".repeat(3 - best.stars)} · ${campaignTime(best.timeMs)}` : done ? t("campaign.noScore") : "☆☆☆";
		...
		const lines = [t("campaign.level", {
			level: id,
			count,
			shape: campaignChapterText(this.levels.find((l) => l.id === id).id).name
		})];
		if (final) {
...
//#endregion
//#region src/game/CampaignChapters.ts
...
var CAMPAIGN_LEVELS = [
	...
	level(6, 19088743, "octahedron", "hard", 18, 4, 5, [
		4,
		4,
		4
	], [
	...
	level(7, 305419896, "cuboid", "hard", 24, 4, 6, [
		5,
		4,
		4
	], [
	...
	level(8, 3735928559, "pyramid", "expert", 28, 4, 6, [
		5,
		5,
		5
	], [
	...
	*/
	...
	build(points, withOutline = true, activeEnd = "head") {
		...
		if (this.fakeTailPlug && this.tailHead && activeEnd === "tail") {
			const joint = points[points.length - 1].clone().addScaledVector(activeExitDirection, -PLUG_HEAD_MAX_LENGTH + PLUG_CABLE_SOCKET_OVERLAP);
			renderPoints.pop();
			while (renderPoints.length > 1 && (renderPoints[renderPoints.length - 1].clone().sub(joint).dot(activeExitDirection) > 0 || renderPoints[renderPoints.length - 1].distanceTo(joint) < CABLE_RADIUS * 2)) renderPoints.pop();
...
//#endregion
//#region src/style/OpeningDepthReveal.ts
var vertexShader$1 = `
  varying vec2 vUv;
  void main() {
    vUv = uv;
...
var OpeningDepthReveal = class {
	blurA = new WebGLRenderTarget(2, 2, { depthBuffer: false });
	blurB = new WebGLRenderTarget(2, 2, { depthBuffer: false });
	...
	revealMaterial = new ShaderMaterial({
		depthTest: false,
		depthWrite: false,
		uniforms: {
			tDiffuse: { value: null },
	...
	blurQuad = new FullScreenQuad(this.blurMaterial);
	revealQuad = new FullScreenQuad(this.revealMaterial);
	...
	setProgress(progress, focusDistance) {
		this.revealMaterial.uniforms.uProgress.value = progress;
		this.revealMaterial.uniforms.uFocusDistance.value = Math.max(.1, focusDistance);
	}
	render(renderer, source, depth, camera, destination) {
		const progress = this.revealMaterial.uniforms.uProgress.value;
		const radius = MathUtils.lerp(1, .25, progress);
...
var SakuraPipeline = class {
	...
	render() {
		...
		if (this.openingDepthReveal) {
			...
			this.openingDepthReveal.render(this.renderer, fxaaTarget.texture, this.sceneTarget.depthTexture, this.camera, revealTarget);
	...
	setOpeningDepthReveal(progress, focusDistance) {
		if (progress >= 1) {
...
var Hud = class {
	...
	firstPlayHint = this.getElement("#first-play-hint");
	seedLabel = this.getElement("#seed-label");
	toast = this.getElement("#toast");
	completePanel = this.getElement("#complete-panel");
	completeStats = this.getElement("#complete-stats");
	completeTime = this.getElement("#complete-time");
	completeMistakes = this.getElement("#complete-mistakes");
	completeStars = this.getElement("#complete-stars");
	completionData = null;
	completeBest = this.getElement("#complete-best");
	...
	showComplete(stats, best, feedback) {
		this.completionData = [
			stats,
			best,
			feedback
		...
		if (stats) {
			...
			const seconds = Math.max(0, Math.round(stats.timeMs / 1e3));
		...
		} else this.completeStats.hidden = true;
		if (best) {
			const seconds = Math.max(0, Math.round(best.timeMs / 1e3));
			...
			const recordLine = feedback?.firstResult ? t("campaign.first", { detail }) : feedback?.isNewRecord ? t("complete.newRecord", { detail }) : detail;
			const nextStar = feedback?.nextStarMistakes && feedback.nextStarMistakes > 0 ? ` · ${t("complete.nextStar", { count: feedback.nextStarMistakes })}` : "";
			this.completeBest.textContent = `${recordLine}${nextStar}`;
			this.completeBest.hidden = false;
		} else this.completeBest.hidden = true;
...
var StartScreen = class {
	...
	campaignButton = this.getElement("#campaign-select-button");
...
var GameSettings = class {
	...
	constructor(toolbar) {
      ...
      <div class="settings-sheet">
		...
		this.move(document.querySelector("#help-strip"), helpSection);
	...
	refreshLocale() {
		...
		const guide = this.dialog.querySelector("#settings-how-to-guide");
		if (guide) guide.innerHTML = english ? "<p><b>Rotate</b> Drag empty space to inspect the bundle.</p><p><b>Find the outside</b> Start with a plug whose exit is clear.</p><p><b>Blocked?</b> Change the angle and check the side or back.</p><p><b>Stars</b> Fewer mistakes earn a better rating.</p>" : "<p><b>旋转</b> 拖动空白处观察线束。</p><p><b>找外层</b> 先找出口畅通的插头。</p><p><b>被挡住</b> 换个角度检查侧面或背面。</p><p><b>星级</b> 错误越少，评价越高。</p>";
	}
	dispose() {
...
var NightEnvironment = class {
	...
	update(delta, elapsed, options) {
		...
		const mobileExploration = this.explorationProgress > 0 && !options.opening && (this.touchTarget || this.coarsePointer.matches);
...
var SeasonController = class {
	...
	appliedThemeProgress = NaN;
	appliedTransitionProgress = NaN;
	appliedTarget = null;
	sourceValue;
	...
	applyDocumentSeason(themeProgress) {
		if (this.appliedThemeProgress === themeProgress && this.appliedTransitionProgress === this.transitionProgress && this.appliedTarget === this.target) return;
...
//#endregion
//#region src/audio/MusicPlayer.ts
var MUSIC_VOLUME = .05;
function selectMusicProfile(opening) {
...
var MusicPlayer = class {
	...
	get diagnostics() {
		return {
			...
			error: this.error,
			playing: [...this.players.entries()].filter(([, player]) => !player.paused).map(([profile, player]) => ({
				profile,
	...
	sync() {
		...
		if (this.profile) {
			...
			if (!player) {
				player = new Audio(`./${tracks[this.profile]}`);
				player.loop = true;
				player.volume = 0;
	...
	tick() {
		const now = performance.now();
		const step = Math.min(.1, (now - this.lastTick) / 1e3) * MUSIC_VOLUME / 1.2;
		this.lastTick = now;
		let fading = false;
		this.players.forEach((player, profile) => {
			...
			if (Math.abs(target - player.volume) > 1e-5) fading = true;
			if (target === 0 && player.volume < 1e-5) {
				player.pause();
...
var INTERACTION_COOLDOWN = .08;
var MAX_INTERACTION_VOICES = 8;
var INTERACTION_FREQUENCIES = {
	confirm: [392, 523],
	"cable-success": [
		440,
		660,
...
var AudioManager = class {
	...
	buses = {};
	listeners = /* @__PURE__ */ new Set();
	interactionVoices = /* @__PURE__ */ new Map();
	lastInteractionAt = /* @__PURE__ */ new Map();
	ambientTarget = -1;
	...
	update(themeProgress) {
		...
		const target = Math.max(0, Math.min(1, themeProgress)) * .115;
		if (this.buses.ambient && Math.abs(target - this.ambientTarget) > 1e-4) {
			this.buses.ambient.gain.setTargetAtTime(target, this.context.currentTime, .18);
	...
	playInteraction(sound) {
		...
		if (sound === "complete" || sound === "failed" || sound === "mode-start") this.stopInteractions();
		else if (this.interactionVoices.size + frequencies.length > MAX_INTERACTION_VOICES) {
			this.droppedInteractions += 1;
			return;
		}
		this.lastInteractionAt.set(sound, now);
		const gainScale = sound === "blocked" ? .1 : sound === "failed" || sound === "rush-warning" ? .09 : sound === "rush-tick" ? .04 : .085;
		const duration = sound === "blocked" ? .18 : sound === "cable-success" ? .28 : sound === "cable-grab" ? .2 : sound === "button" ? .14 : .13;
		frequencies.forEach((frequency, index) => {
			this.scheduleTone(this.buses.interaction, frequency, now + index * .09, duration, gainScale, sound === "blocked" ? "triangle" : "sine");
		});
	}
	stopInteractions() {
		if (!this.context) return;
		...
		this.interactionVoices.forEach((gain, source) => {
			...
			gain.gain.setTargetAtTime(1e-4, now, .006);
			source.stop(now + .03);
		});
...
var Game = class {
	...
	nightEnvironment;
	adaptiveQuality;
	campaignProgress = new CampaignProgress(CAMPAIGN_LEVELS.length);
	campaignUi = new CampaignUi(this.campaignProgress, CAMPAIGN_LEVELS, (id) => this.loadCampaignLevelById(id), () => this.resetCurrentPuzzle());
	...
	startScreen = new StartScreen({
		...
		campaignStartLabel: () => {
			...
			return snapshot.completed.length === 0 ? t("start.enter") : t(snapshot.allComplete ? "campaign.revisit" : "campaign.continue", { level: snapshot.nextLevel });
		},
	...
	homeDepthRevealStartedAt = null;
	homeDepthRevealProgress = 0;
	homeDepthRevealComplete = false;
	homeDepthFocus = new Vector3();
	...
	constructor(canvas) {
		...
		this.puzzleWorker = new Worker(new URL(
			/* @vite-ignore */
			"" + new URL("generator.worker-BulJklvP.js", import.meta.url).href,
			"" + import.meta.url
		), { type: "module" });
	...
	async compileSceneAsync(object, camera, targetScene = null) {
		const context = this.renderer.getContext();
		if (Boolean(context?.getExtension?.("KHR_parallel_shader_compile"))) {
	...
	async warmPerformanceEffectsAsync() {
		...
		try {
			for (const root of roots) await this.compileSceneAsync(root, warmupCamera, this.scene);
			this.renderer.setRenderTarget(this.applianceGeometryWarmupTarget);
			this.renderer.clear();
	...
	startCampaignFromOpening() {
		...
		if (this.puzzle && this.currentMode === "campaign" && this.currentLevel?.id === id && this.puzzle.seed === seedForCampaignLevel(id)) this.beginOpeningTransition();
		else this.loadCampaignLevelById(id);
	}
	loadCampaignLevelById(levelId) {
	...
	showPuzzleCompletion() {
		...
		if (this.currentMode === "campaign" && this.currentLevel) {
			...
			const nextStarMistakes = result.stars === 2 ? result.mistakes : result.stars === 1 ? result.mistakes - 2 : void 0;
			this.hud.showComplete(result, outcome.best, {
				isNewRecord: outcome.status === "record",
				firstResult: outcome.status === "first",
				nextStarMistakes
			});
	...
	async applyPuzzle(puzzle, staged = false, preserveRandomLives = false, applyToken = this.puzzleApplyToken) {
		...
		const configuredDefinitions = this.currentMode === "campaign" && this.currentLevel ? selectCampaignAppliances(campaignChapter(this.currentLevel.id).appliances, activeColors.length) : void 0;
		...
		this.appliances.setRequiredColors(this.currentMode === "rush" ? [] : this.arrows.map((arrow) => arrow.definition.color));
		const deferInitialWarmups = staged && this.openingActive && this.currentMode === "campaign" && this.currentLevel?.id === 1;
		if (this.currentMode !== "rush") {
	...
	*/
	...
	beginTelevisionReconstruction(cableIds, sourceTarget) {
		...
		this.compileSceneAsync(this.televisionReconstruction.root, this.camera, this.scene);
	}
	beginToasterEndSwap(cableIds, sourceTarget) {
		...
		this.compileSceneAsync(this.toasterHeatSwap.root, this.camera, this.scene);
	}
	...
	finishSkillResolution(topologyChanged, presentationDuration, stateAlreadySynced = false) {
		...
		const settleWhenReady = () => {
			...
			this.applianceRoutingRevision = this.appliances.routingRevision;
			this.refreshAvailability(false);
			if (remaining === 0 && this.connections.activeCount === 0) this.showPuzzleCompletion();
		};
	...
	completeConnection(target, arrow) {
		...
		window.setTimeout(() => {
			...
			if (this.arrows.length - this.removedCount === 0 && this.connections.activeCount === 0) this.showPuzzleCompletion();
		}, 0);
	}
	...
	updateHomeDepthReveal() {
		if (this.homeDepthRevealComplete) return;
		if (!this.openingActive || this.openingTransitioning || this.openingStartPending || this.explorationMode || this.applianceGallery?.visible || this.contextUnavailable || this.theme.snapshot.reducedMotion) this.homeDepthRevealProgress = 1;
