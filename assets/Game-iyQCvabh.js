...
import { t as ThemeController } from "./game-BFsISjB0.js";
...
var LandscapeControl = class {
	...
	refresh = () => {
		...
		this.button.textContent = this.requested ? english ? "Exit landscape mode" : "退出横屏模式" : english ? "Play in landscape" : "横屏游玩";
		this.button.setAttribute("aria-pressed", String(this.requested));
		this.button.disabled = this.busy;
		this.status.hidden = !this.requested;
		this.status.textContent = innerWidth > innerHeight ? english ? "Landscape layout active · appliances on both sides" : "横屏布局已启用 · 家电位于左右两侧" : english ? "Turn your phone sideways. If it stays upright, disable rotation lock." : "请将手机横过来；若画面未旋转，请关闭手机的竖屏锁定。";
	};
	toggle = async () => {
		if (this.busy) return;
		if (this.requested) {
