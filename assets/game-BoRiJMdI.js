...
async function boot() {
	if (showcaseMode === "plugs" || showcaseMode === "appliances") {
		...
		const { Showcase } = await __vitePreload(async () => {
			const { Showcase } = await import("./Showcase-DV_mhqOI.js");
			return { Showcase };
		...
		const { Game } = await __vitePreload(async () => {
			const { Game } = await import("./Game-BCqPVZfO.js");
			return { Game };
