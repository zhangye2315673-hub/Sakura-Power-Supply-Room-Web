...
async function boot() {
	if (showcaseMode === "plugs" || showcaseMode === "appliances") {
		...
		const { Showcase } = await __vitePreload(async () => {
			const { Showcase } = await import("./Showcase-BHXnac9s.js");
			return { Showcase };
		...
		const { Game } = await __vitePreload(async () => {
			const { Game } = await import("./Game-CB7u6rDy.js");
			return { Game };
