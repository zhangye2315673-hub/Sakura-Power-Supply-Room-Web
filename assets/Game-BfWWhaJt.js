...
import { t as ThemeController } from "./game-DblucdZP.js";
...
//#endregion
//#region src/systems/LandscapeControl.ts
var LandscapeControl = class {
	...
	constructor() {
		this.element.className = "landscape-control";
		this.button.id = "landscape-mode-button";
		this.button.type = "button";
		this.status.id = "landscape-mode-status";
	...
	refresh = () => {
		...
		this.element.hidden = !matchMedia("(pointer: coarse)").matches && navigator.maxTouchPoints === 0;
		this.button.textContent = this.requested && innerWidth <= innerHeight ? english ? "Cancel landscape switch" : "取消横屏切换" : this.requested ? english ? "Exit landscape mode" : "退出横屏模式" : english ? "Play in landscape" : "横屏游玩";
		this.button.setAttribute("aria-pressed", String(this.requested && innerWidth > innerHeight));
		this.button.disabled = this.busy;
		this.status.hidden = !this.requested;
		this.status.textContent = innerWidth > innerHeight ? english ? "Landscape layout active · appliances on both sides" : "横屏布局已启用 · 家电位于左右两侧" : english ? "Turn your phone sideways. If it stays upright, disable rotation lock." : "浏览器未能自动横屏。请关闭手机的竖屏锁定，再将手机横过来；游戏会自动改为左右布局。";
	};
	toggle = async () => {
		if (this.busy) return;
		if (this.requested) {
			...
			if (this.ownsFullscreen && document.fullscreenElement) await document.exitFullscreen().catch(() => {});
		...
		try {
			...
			if (innerWidth <= innerHeight) await new Promise((resolve) => window.setTimeout(resolve, 650));
			if (innerWidth <= innerHeight && this.ownsFullscreen) {
				this.ownsFullscreen = false;
				this.orientation?.unlock?.();
	...
	dispose() {
		...
		if (this.requested) this.orientation?.unlock?.();
		if (this.ownsFullscreen && document.fullscreenElement) document.exitFullscreen().catch(() => {});
		this.element.remove();
	}
};
//#endregion
...
var GameSettings = class {
	...
	restoredElements = [];
	landscape = new LandscapeControl();
	constructor(toolbar) {
      ...
      <div class="settings-sheet">
		...
		this.dialog.querySelector("#settings-preferences").append(this.landscape.element);
