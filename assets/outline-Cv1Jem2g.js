...
import { E as jelly, n as mergeVertices } from "./BufferGeometryUtils-CtH-hlZf.js";
...
var SMALL_SHADOW_RADIUS = .12;
var SHADOW_CRITICAL_PART = /(?:body|shell|base|cabinet|housing|casing|enclosure|panel|door|lid|chamber|globe|guard|frame|ring|barrel|tower|handle|wheel|foot)/i;
function optimizeApplianceShadowCasters(root) {
	let before = 0;
	let after = 0;
	root.traverse((object) => {
		...
		if ((geometry.boundingSphere?.radius ?? Number.POSITIVE_INFINITY) * Math.max(object.scale.x, object.scale.y, object.scale.z) >= SMALL_SHADOW_RADIUS || SHADOW_CRITICAL_PART.test(name) || SHADOW_CRITICAL_PART.test(part) || object.userData.shadowCritical === true) after += 1;
