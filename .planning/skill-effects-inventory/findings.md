...
| 13 | 咖啡机 / `coffee-lock` 咖啡封技 | 无 DEBUFF 时生成 3 回合 DEBUFF；遮蔽线色并阻止后续 3 次家电技能。 | 无咖啡机专属 3D 技能资产；全屏 `coffee-lock` 棕色液体覆盖；全线咖啡色 tint。 | DEBUFF `coffee-lock`，3 回合。 | cue；每次被封锁仍提示“技能被封锁”；状态 tooltip。 | “封技”反馈应和普通无效果区分；全线 tint 不能降低可抽性判断。 |
